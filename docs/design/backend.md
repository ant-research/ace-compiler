...
| acelib | cuda | libFHErt_ace | High-performance GPU FHE (H100/sm_90) |
...
**Files**: `backend/acelib.py`

```python
class AcelibLIB(Backend):
    """Acelib GPU backend optimized for H100."""
...
| libant_cpu.so / libacelib_gpu.so | PUBLIC libcommon | Backend implementation library, dependent on the common layer |
...
register_backend("acelib", "cuda", AcelibLIB)
