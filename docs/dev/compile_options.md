...
| 组件 | 文件 | 职责 |
|------|------|------|
| `CompileOptions` | `python/ace/fhe/config/compile_options.py` | 定义选项字段，验证参数 |
| `_dict_to_cmd_args()` | `python/ace/fhe/config/compile_options.py` | 将字典转换为命令行参数 |
| `decorators.py` | `python/ace/fhe/decorators.py` | 接收用户选项，传递给 backend |
| `Backend` | `python/ace/fhe/backend/antlib.py` | 存储选项，调用编译器 |
...
| 文件 | 说明 |
|------|------|
| `python/ace/fhe/config/compile_options.py` | CompileOptions 定义和转换函数 |
| `python/ace/fhe/decorators.py` | fhe.compile/compute 装饰器 |
| `python/ace/fhe/backend/antlib.py` | AntLIB backend 实现 |
