# tests/regression/sample/test_smoke_antlib.py
"""
Antlib (CPU) FHE smoke tests: compilation and inference.

Tests FHE compilation and inference for built-in ops and functions
on the antlib-cpu backend. These tests are slow — use -m "not slow" to skip.

Run examples:
    pytest tests/regression/sample/test_smoke_antlib.py            # all antlib tests
    pytest tests/regression/sample/test_smoke_antlib.py -k "op"    # ops only
    pytest tests/regression/sample/test_smoke_antlib.py -k "func"  # funcs only
...
#   conv2d_relu_op — FHERuntime inference segfaults in executor
...
def test_op_compile(spec):
    """Op FHE compilation on antlib-cpu."""
...
def test_op_smoke(spec):
    """Op FHE inference on antlib-cpu."""
    prog = _compile_op(spec)
    assert prog is not None
...
def test_func_compile(spec):
    """Function FHE compilation on antlib-cpu."""
...
def test_func_smoke(spec):
    """Function FHE inference on antlib-cpu."""
    prog = _compile_func(spec)
    assert prog is not None
