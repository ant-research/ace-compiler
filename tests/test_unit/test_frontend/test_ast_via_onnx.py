...
"""
Unit tests for ast-via-onnx frontend.
...
class TestPrepare:
    ...
    def test_ir_format_type_is_file(self, frontend, model, model_inputs):
        """Test that format_type is 'file'."""
        result = frontend.prepare(model, model_inputs, ["x"])
        assert result.format_type == "file"

    def test_ir_file_format_is_onnx(self, frontend, model, model_inputs):
        """Test that file_format is 'onnx'."""
        result = frontend.prepare(model, model_inputs, ["x"])
        assert result.file_format == "onnx"

    def test_with_complex_function(self, frontend):
        """Test prepare() with complex function."""
        inputs = [torch.randn(1, 4), torch.randn(1, 4)]
        result = frontend.prepare(add_func, inputs, ["x", "y"])
        assert isinstance(result, ONNXFileIR)
...
class TestCompile:
    """Tests for compile() method - Memory mode (NOT IMPLEMENTED)."""

    def test_raises_not_implemented(self, frontend, model, model_inputs):
        """Test that compile() raises NotImplementedError."""
        with pytest.raises(NotImplementedError) as exc_info:
            frontend.compile(model, model_inputs, ["x"])
        msg = str(exc_info.value).lower()
        assert "memory" in msg or "not implemented" in msg
...
class TestExportOnnx:
    ...
    @pytest.mark.skip(reason="ast-via-onnx ONNX export not working")
...
class TestExportAir:
    ...
    @pytest.mark.skip(reason="ast-via-onnx AIR export not working")
...
class TestEdgeCases:
    ...
    def test_prepare_with_empty_inputs(self, frontend):
        """Test prepare() with empty inputs list raises AssertionError."""
        with pytest.raises(AssertionError):
            frontend.prepare(add_func, [], ["x"])

    def test_prepare_with_wrong_input_count(self, frontend):
        """Test prepare() with wrong number of inputs."""
        inputs = [torch.randn(1, 4)]
        with pytest.raises((TypeError, RuntimeError)):
            frontend.prepare(add_func, inputs, ["x"])
...
class TestFrontendMeta:
    ...
    def test_frontend_name(self, frontend):
        """Test frontend name."""
        assert frontend.name() == "ast-via-onnx"
