...
#undef DECL_RTM
  ...
  // Print totals for operations split across bootstrap/non-bootstrap
  typedef struct {
    const char*     name;
    RTLIB_TIMING_ID non_bs;
    RTLIB_TIMING_ID bs;
  } TOTAL_PAIR;
  TOTAL_PAIR pairs[] = {
    {"PRECOMP",  RTM_PRECOMP,  RTM_BS_PRECOMP},
    {"MOD_DOWN", RTM_MOD_DOWN, RTM_BS_MOD_DOWN},
    {"DOT_PROD", RTM_DOT_PROD, RTM_BS_DOT_PROD},
  };
  bool has_total = false;
  for (size_t i = 0; i < sizeof(pairs) / sizeof(pairs[0]); i++) {
    uint64_t total_count =
        Rtlib_count[pairs[i].non_bs] + Rtlib_count[pairs[i].bs];
    if (total_count > 0 && Rtlib_count[pairs[i].bs] > 0) {
      if (!has_total) {
        fprintf(fp, "%-24s\t%12s\t%12s\n", "Total (non-bs + bs)", "Count",
                "Elapse");
        fprintf(fp, "%-24s\t%12s\t%12s\n", LONG_BAR, SHORT_BAR, SHORT_BAR);
        has_total = true;
      }
      uint64_t total_time =
          Rtlib_timing[pairs[i].non_bs] + Rtlib_timing[pairs[i].bs];
      fprintf(fp, "%-24s\t%12ld\t%12.6f sec\n", pairs[i].name, total_count,
