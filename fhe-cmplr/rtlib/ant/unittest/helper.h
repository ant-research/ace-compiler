...
static inline void Calculate_approximation_error(DCMPLX* result, DCMPLX* expect,
                                                 int len) {
  // using the Euclidean norm
  double avrg = 0;
  for (size_t i = 0; i < len; ++i) {
    avrg += pow(fabs(result[i].real() - expect[i].real()), 2);
  }

  avrg = sqrt(avrg) / len;  // get the average
...
static inline void Check_complex_vector_approx_eq(
  ...
  Calculate_approximation_error(Get_dcmplx_values(vec1),
                                Get_dcmplx_values(vec2), vec1_len);
  bool is_diff = false;
