...
int64_t* Hw_modmul_fast(int64_t* res, int64_t* val1, int64_t* val2,
                        int64_t* val2_inv, MODULUS* modulus, uint32_t degree) {
  RTLIB_TM_START(RTM_HW_MUL, rtm);
  int64_t mod = Get_mod_val(modulus);
  for (uint32_t idx = 0; idx < degree; idx++) {
    *res = Fast_mul_const_with_mod(*val1, *val2, *val2_inv, mod);
    val1++;
    val2++;
    val2_inv++;
    res++;
