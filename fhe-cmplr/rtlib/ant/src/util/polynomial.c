...
void Multiply_add_fast(POLYNOMIAL* res, POLYNOMIAL* poly1, POLYNOMIAL* poly2,
                       VALUE_LIST* q_primes, VALUE_LIST* p_primes) {
  ...
  for (size_t module_idx = 0; module_idx < Get_poly_level(res); module_idx++) {
    for (uint32_t idx = 0; idx < Get_rdgree(res); idx++) {
      int64_t tmp = Fast_mul_const_with_mod(
          *poly1_data, *poly2_data, *poly2_data, Get_mod_val(q_modulus));
  ...
  if (p_primes) {
    ...
    for (size_t module_idx = 0; module_idx < Get_num_p(res); module_idx++) {
      for (uint32_t idx = 0; idx < Get_rdgree(res); idx++) {
        int64_t tmp = Fast_mul_const_with_mod(
            *poly1_data, *poly2_data, *poly2_data, Get_mod_val(p_modulus));
...
POLYNOMIAL* Multiply_plain_fast(POLYNOMIAL* res, POLYNOMIAL* poly1,
                                POLYNOMIAL* poly2, POLYNOMIAL* poly2_prec,
                                VALUE_LIST* q_primes, VALUE_LIST* p_primes) {
  IS_TRUE(Is_ntt(res) && Is_ntt(poly1) && Is_ntt(poly2) && Is_ntt(poly2_prec),
          "opnd/res is not ntt");
  IS_TRUE(Is_size_match(res, poly1) && Is_size_match(res, poly2) &&
...
POLYNOMIAL* Precomp_poly(POLYNOMIAL* res, POLYNOMIAL* poly,
                         VALUE_LIST* q_primes, VALUE_LIST* p_primes) {
  IS_TRUE(Is_size_match(res, poly), "size not match");
  IS_TRUE(Get_poly_level(res) <= LIST_LEN(q_primes), "primes not match");
  int64_t* res_coeffs  = Get_poly_coeffs(res);
  int64_t* poly_coeffs = Get_poly_coeffs(poly);
  ...
  if (p_primes) {
    res_coeffs  = Get_p_coeffs(res);
    poly_coeffs = Get_p_coeffs(poly);
    modulus     = Get_modulus_head(p_primes);
    for (size_t module_idx = 0; module_idx < Get_num_p(res); module_idx++) {
      int64_t mod_val = Get_mod_val(modulus);
      for (uint32_t poly_idx = 0; poly_idx < Get_rdgree(res); poly_idx++) {
        *res_coeffs = Precompute_const(*poly_coeffs, mod_val);
...
POLYNOMIAL* Fast_dotprod_poly(POLYNOMIAL* res, POLYNOMIAL** p0, POLYNOMIAL** p1,
                              VALUE_LIST* q_primes, VALUE_LIST* p_primes,
                              uint32_t num) {
  IS_TRUE(Is_ntt(res) && Is_ntt(p0[0]) && Is_ntt(p1[0]), "opnd/res is not ntt");
...
POLYNOMIAL* Dotprod_poly(POLYNOMIAL* res, POLYNOMIAL** p0, POLYNOMIAL** p1,
                         VALUE_LIST* q_primes, VALUE_LIST* p_primes,
                         uint32_t num) {
  for (uint32_t idx = 0; idx < num; idx++) {
    Multiply_add(res, p0[idx], p1[idx], q_primes, p_primes);
  }
  return res;
}
...
void Derive_poly(POLYNOMIAL* res, POLYNOMIAL* poly, size_t q_cnt,
                 size_t p_cnt) {
  FMT_ASSERT(
      poly && res && q_cnt <= Get_poly_level(poly) && p_cnt <= Get_num_p(poly),
      "invalid range");
...
void Fast_base_conv(POLYNOMIAL* new_poly, POLYNOMIAL* old_poly,
  ...
  for (size_t n_idx = 0; n_idx < new_level; n_idx++) {
    ...
    VALUE_LIST* mod_nb      = Is_q(old_primes)
                                  ? Get_qhatmodp_at(old_primes, old_level - 1, n_idx)
                                  : Get_phatmodq_at(old_primes, n_idx);
    for (uint32_t d_idx = 0; d_idx < ring_degree; d_idx++) {
      INT128_T sum = 0;
      for (size_t o_idx = 0; o_idx < old_level; o_idx++) {
...
void Reduce_rns_base(POLYNOMIAL* new_poly, POLYNOMIAL* old_poly,
                     CRT_CONTEXT* crt) {
  if (new_poly->_data == NULL) {
    Alloc_poly_data(new_poly, Get_rdgree(old_poly), Get_num_q(old_poly), 0);
  }
  CRT_PRIMES* q_primes    = Get_q(crt);
  CRT_PRIMES* p_primes    = Get_p(crt);
  ...
  // changed after Reduce_rns_base
  POLYNOMIAL old_poly_p_part, p_poly;
  Extract_poly(&old_poly_p_part, old_poly, Get_num_q(new_poly), p_prime_len);
  Alloc_poly_data(&p_poly, Get_rdgree(&old_poly_p_part),
                  Get_num_q(&old_poly_p_part), 0);
  Copy_polynomial(&p_poly, &old_poly_p_part);

  if (Is_ntt(old_poly)) {
    Conv_ntt2poly_inplace_with_primes(&p_poly, crt, Get_p_primes(crt));
  }
  Fast_base_conv(new_poly, &p_poly, q_primes, p_primes);
  if (Is_ntt(old_poly)) {
...
#if 1
POLYNOMIAL* Reduce_rotate_and_rescale(POLYNOMIAL* res, int32_t rot_idx,
                                      POLYNOMIAL* poly, CRT_CONTEXT* crt,
                                      bool output_ntt) {
  CRT_PRIMES* q_primes    = Get_q(crt);
  CRT_PRIMES* p_primes    = Get_p(crt);
  size_t      num_p       = Get_num_p(poly);
  ...
  IS_TRUE(poly && res && ring_degree == Get_rdgree(poly) &&
              num_q == Get_num_q(res) && num_q > 1 &&
              Get_primes_cnt(p_primes) == num_p && Get_num_p(res) == 0,
  ...
  // calculate rotate order
  MODULUS two_n_modulus;
  Init_modulus(&two_n_modulus, 2 * ring_degree);
  uint32_t    k       = Find_automorphism_index(rot_idx, &two_n_modulus);
  VALUE_LIST* precomp = Alloc_value_list(I64_TYPE, ring_degree);
  Precompute_automorphism_order(precomp, k, ring_degree, Is_ntt(poly));
  int64_t* precomp_data = Get_i64_values(precomp);

  POLYNOMIAL poly_p_part;
  Extract_poly(&poly_p_part, poly, Get_num_q(poly), num_p);
  ...
  for (uint32_t d_idx = 0; d_idx < ring_degree; d_idx++) {
    // int64_t map_idx = precomp_data[d_idx];
    // int64_t last_old_rot = map_idx >=0 ? last_old[map_idx] : last_qi -
    // last_old[-map_idx];
    last_new[d_idx] =
        Sub_fast_mul_mod(last_old[d_idx], last_new[d_idx], last_p_m_inv,
  ...
  for (size_t q_idx = 0; q_idx < last_idx; q_idx++) {
    ...
    uint64_t offset             = q_idx * ring_degree;
    for (uint32_t d_idx = 0; d_idx < ring_degree; d_idx++) {
      int64_t map_idx      = precomp_data[d_idx];
      int64_t last_old_rot = map_idx >= 0 ? old_data[offset + map_idx]
                                          : qi - old_data[offset - map_idx];
      FMT_ASSERT(last_old_rot < qi && last_old_rot >= 0, "invalid range");
      int64_t new_val = Sub_fast_mul_mod(*old_data, *new_data, p_m_inv_modqi,
                                         p_m_inv_modqi_prec, qi);
      int64_t last_new_rot =
          map_idx >= 0 ? last_new[map_idx] : last_qi - last_new[-map_idx];
      *new_data =
          Sub_fast_mul_mod(new_val, last_new_rot, ql_inv, ql_inv_prec, qi);
      new_data++;
      old_data++;
    }
  }
...
POLYNOMIAL* Reduce_and_rescale(POLYNOMIAL* res, POLYNOMIAL* poly,
                               CRT_CONTEXT* crt, bool output_ntt) {
  CRT_PRIMES* q_primes    = Get_q(crt);
  CRT_PRIMES* p_primes    = Get_p(crt);
  size_t      num_p       = Get_num_p(poly);
  size_t      num_q       = Get_num_q(poly);
  ...
  for (size_t q_idx = 0; q_idx < last_idx; q_idx++) {
    ...
    for (uint32_t d_idx = 0; d_idx < ring_degree; d_idx++) {
      ...
      int64_t new_val = Sub_fast_mul_mod(*old_data, *new_data, p_m_inv_modqi,
                                         p_m_inv_modqi_prec, qi);
      *new_data =
          Sub_fast_mul_mod(new_val, last_new[d_idx], ql_inv, ql_inv_prec, qi);
#else
      int64_t   cst1 = Mul_int64_with_mod(p_m_inv_modqi, ql_inv, qi);
      int64_t   val1 = Sub_int64_with_mod(*old_data, *new_data, qi);
      UINT128_T mul1 = (UINT128_T)val1 * (UINT128_T)cst1;
      UINT128_T mul2 = (UINT128_T)last_new[d_idx] * (UINT128_T)ql_inv;
      INT128_T  sub  = mul1 - mul2;
      if (mul1 < mul2) {
        sub = sub * -1;
      }
      // FMT_ASSERT(mul1 >= mul2, "invalid");
      int64_t res = Mod_barrett_128(sub, q_modulus);
      if (mul1 < mul2) {
        res = res * -1 + qi;
      }
      *new_data = res;
...
VALUE_LIST* Reduce_and_rescale_modup(POLYNOMIAL* res, POLYNOMIAL* poly,
  ...
  size_t      num_decomp       = Get_num_decomp(num_q - 1, crt);
  VALUE_LIST* poly_raised_list = Alloc_value_list(PTR_TYPE, num_decomp);
  Alloc_poly_list(poly_raised_list, ring_degree, num_q - 1, num_p);
  ...
  for (size_t q_idx = 0; q_idx < last_idx; q_idx++) {
    ...
    for (uint32_t d_idx = 0; d_idx < ring_degree; d_idx++) {
      int64_t new_val = Sub_fast_mul_mod(*old_data, *res_data, p_m_inv_modqi,
                                         p_m_inv_modqi_prec, qi);
      new_val =
          Sub_fast_mul_mod(new_val, last_new[d_idx], ql_inv, ql_inv_prec, qi);
    ...
    if (part_idx == cur_part_size - 1) {
      Modup_inplace(raised, &part2_poly, crt, num_decomp, decomp_idx);
      decomp_idx++;
      part_idx      = 0;
...
VALUE_LIST* Switch_key_precompute_fusion2(POLYNOMIAL* poly, CRT_CONTEXT* crt) {
  CRT_PRIMES* q_parts    = Get_qpart(crt);
  size_t      num_q      = Get_num_q(poly);
  size_t      num_p      = Get_crt_num_p(crt);
  size_t      num_decomp = Get_num_decomp_poly(poly, crt);
  ...
  FOR_ALL_ELEM(poly_raised_list, decomp_idx) {
    POLYNOMIAL* raised =
        (POLYNOMIAL*)Get_ptr_value_at(poly_raised_list, decomp_idx);
    Decompose_modup2(raised, poly, crt, num_decomp, decomp_idx);
  }
  return poly_raised_list;
}
...
void Decompose_modup2(POLYNOMIAL* raised, POLYNOMIAL* poly, CRT_CONTEXT* crt,
                      size_t num_decomp, size_t decomp_idx) {
  uint32_t     degree    = Get_rdgree(poly);
  CRT_PRIMES*  q_parts   = Get_qpart(crt);
  size_t       part_size = Get_per_part_size(q_parts);
  ...
  for (size_t n = 0; n < degree; n++) {
    for (size_t i = 0; i < num_part2; i++) {
      int64_t  val = Get_coeff_at(&part2_poly, i * degree + n);
      MODULUS* qi  = (MODULUS*)Get_modulus(Get_vlprime_at(part2_primes, i));
      int64_t  mul_inv_modq =
          Mul_int64_mod_barret(val, Get_i64_value_at(ql_hatinvmodq, i), qi);
      VALUE_LIST* qhat_modp = VL_VALUE_AT(ql_hatmodp, i);
  ...
  Set_is_ntt(raised, false);
#else
  INT128_T* sum = (INT128_T*)malloc(sizeof(INT128_T) * degree * num_compl);
  memset(sum, 0, sizeof(INT128_T) * degree * num_compl);
  for (size_t i = 0; i < num_part2; i++) {
    int64_t*    part2_coeffs = Get_poly_coeffs_at(&part2_poly, i);
    MODULUS*    qi = (MODULUS*)Get_modulus(Get_vlprime_at(part2_primes, i));
    int64_t     ql_hatinv = Get_i64_value_at(ql_hatinvmodq, i);
    VALUE_LIST* qhat_modp = VL_VALUE_AT(ql_hatmodp, i);
    for (size_t n = 0; n < degree; n++) {
      int64_t val          = part2_coeffs[n];
      int64_t mul_inv_modq = Mul_int64_mod_barret(val, ql_hatinv, qi);
      for (size_t j = 0; j < num_compl; j++) {
        sum[j * degree + n] +=
            (INT128_T)mul_inv_modq * Get_i64_value_at(qhat_modp, j);
      }
    }
  }

  for (size_t j = 0; j < num_part1; j++) {
    MODULUS*  part1_modulus = Get_modulus(Get_vlprime_at(&part1_primes, j));
    int64_t*  part1_coeffs  = Get_poly_coeffs_at(raised, j);
    INT128_T* sumi          = sum + j * degree;
    for (size_t n = 0; n < degree; n++) {
      part1_coeffs[n] = Mod_barrett_128(sumi[n], part1_modulus);
    }
  }

  for (size_t j1 = 0, j2 = num_part1; j1 < num_part3; j1++, j2++) {
    MODULUS*  part3_modulus = Get_modulus(Get_vlprime_at(&part3_primes, j1));
    int64_t*  part3_coeffs  = Get_poly_coeffs_at(raised, j1 + part3_start_idx);
    INT128_T* sumi          = sum + j2 * degree;
    for (size_t n = 0; n < degree; n++) {
      part3_coeffs[n] = Mod_barrett_128(sumi[n], part3_modulus);
    }
  }
  Set_is_ntt(raised, false);
#endif
...
void Modup_inplace(POLYNOMIAL* raised, POLYNOMIAL* part2_poly, CRT_CONTEXT* crt,
                   size_t num_decomp, size_t decomp_idx) {
  uint32_t     degree    = Get_rdgree(raised);
  CRT_PRIMES*  q_parts   = Get_qpart(crt);
  size_t       part_size = Get_per_part_size(q_parts);
  size_t       num_q     = Get_num_q(raised);
  VL_CRTPRIME* compl_primes =
      Get_qpart_compl_at(Get_qpart_compl(crt), num_q - 1, decomp_idx);
  VL_CRTPRIME* part2_primes = Get_vl_value_at(Get_primes(q_parts), decomp_idx);
  ...
  size_t num_part2       = Get_num_q(part2_poly);
  size_t num_part3       = Get_num_pq(raised) - num_part1 - num_part2;
  size_t num_compl       = LIST_LEN(compl_primes);
  size_t part3_start_idx = num_part1 + num_part2;
...
void Modup_inplace_2(POLYNOMIAL* raised, POLYNOMIAL* part2_poly,
                     int64_t* arr_mul_inv_modq, CRT_CONTEXT* crt,
                     size_t num_decomp, size_t decomp_idx) {
  uint32_t     degree    = Get_rdgree(raised);
  CRT_PRIMES*  q_parts   = Get_qpart(crt);
  size_t       part_size = Get_per_part_size(q_parts);
...
VALUE_LIST* Switch_key_precompute(POLYNOMIAL* poly, CRT_CONTEXT* crt) {
  uint64_t rtm_bs = 0;
  if (Rtlib_in_bootstrap) rtm_bs = Mark_rtm_start();
  VALUE_LIST* result;
  if (Get_rtlib_config(CONF_OP_FUSION_DECOMP_MODUP)) {
    result = Switch_key_precompute_fusion(poly, crt);
  } else {
    result = Switch_key_precompute_no_fusion(poly, crt);
  }
  if (Rtlib_in_bootstrap) Mark_rtm_end(RTM_BS_PRECOMP, rtm_bs);
  return result;
}

VALUE_LIST* Switch_key_precompute2(POLYNOMIAL* poly, CRT_CONTEXT* crt) {
  if (Get_rtlib_config(CONF_OP_FUSION_DECOMP_MODUP)) {
    return Switch_key_precompute_fusion2(poly, crt);
  } else {
