...
CIPHERTEXT* Eval_bootstrap(CIPHERTEXT* res, CIPHERTEXT* ciph,
  ...
  // Verify whether a bootstrap is required
  // Step1: get level_budget from precom
  ...
  // Step2: obtain the depth of bootstrapping consumption
  uint32_t bts_depth =
      Get_bootstrap_depth(level_budget, params->_hamming_weight);
  Free_value_list(level_budget);
  // Step3: set level_after_bts to the maximum level if it has not been set
  if (!level_after_bts) level_after_bts = q_cnt - bts_depth;
  IS_TRUE(level_after_bts <= q_cnt - bts_depth,
          "The level set after bootstrapping is excessively high");
  // Step4: if we start with more towers than we obtain from bootstrapping,
  ...
  uint32_t raise_level = level_after_bts + bts_depth;
