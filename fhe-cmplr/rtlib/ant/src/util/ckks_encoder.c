...
void Encode_at_level_with_sf(PLAINTEXT* res, CKKS_ENCODER* encoder,
                             VALUE_LIST* values, uint32_t level, uint32_t slots,
                             uint32_t sf_degree, uint32_t p_cnt) {
  Encode_impl(res, encoder, values, level, slots, sf_degree, p_cnt);
}
...
void Encode_val_to_poly(POLYNOMIAL* poly, CKKS_ENCODER* encoder, double value,
                        double scale) {
  CRT_CONTEXT* crt   = Get_param_crt(encoder->_params);
  uint32_t     q_cnt = Get_primes_cnt(Get_q(crt));
  uint32_t     p_cnt = Get_num_p(poly);
  uint32_t     level = Get_poly_level(poly);
  FMT_ASSERT(level > 0 && level <= q_cnt,
             "level should not be larger than q_cnt");
  FMT_ASSERT(p_cnt == 0 || p_cnt == Get_primes_cnt(Get_p(crt)),
             "invalid p primes count");
  FMT_ASSERT(scale > 0 && (uint32_t)log2(scale) <
                              Get_coeff_bit_count(level, encoder->_params),
             "invalid scale for encode");
  ...
  uint32_t sf_bit          = (uint32_t)log2(Get_param_sc(encoder->_params));
  uint32_t q0_bit          = encoder->_params->_first_mod_size;
  FMT_ASSERT(
  ...
  poly_data    = Get_p_coeffs(poly);
  modulus_head = Get_modulus_head(Get_p_primes(crt));
  modulus      = modulus_head;
  for (uint32_t i = 0; i < p_cnt; i++) {
    int64_t mod     = Get_mod_val(modulus);
    int64_t res_val = Mod_int64(val_int, mod);
    for (uint32_t idx = 0; idx < degree; idx++) {
      *poly_data = res_val;
      poly_data++;
    }
  ...
  if (log_approx > 0) {
    FMT_ASSERT(p_cnt == 0, "not supported for extend poly");
...
void Encode_val_at_level(PLAINTEXT* res, CKKS_ENCODER* encoder, double value,
                         uint32_t level, uint32_t sf_degree, bool is_ext) {
  ...
  uint32_t     q_cnt          = Get_primes_cnt(Get_q(crt));
  uint32_t     p_cnt          = is_ext ? Get_primes_cnt(Get_p(crt)) : 0;
  if (level == 0) level = q_cnt;
  FMT_ASSERT(level <= q_cnt, "level should not be larger than mul_depth + 1");
  ...
  double scale = pow(scaling_factor, sf_degree);
  Init_plaintext(res, degree, Get_default_slot_size(encoder), level, p_cnt,
                 scale, sf_degree);
