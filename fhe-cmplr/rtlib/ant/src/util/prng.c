...
//! @brief Free a BLAKE2_PRNG instance
void Free_blake2_prng(BLAKE2_PRNG* prng) {
  if (prng == NULL) return;
  if (prng->_seed) {
    Free_value_list(prng->_seed);
    prng->_seed = NULL;
  }
  if (prng->_buffer) {
    Free_value_list(prng->_buffer);
    prng->_buffer = NULL;
  }
  free(prng);
}
