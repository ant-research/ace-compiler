...
CIPHERTEXT* Mul_const(CIPHERTEXT* res, CIPHERTEXT* ciph, double const_val,
                      CKKS_EVALUATOR* eval) {
  PLAINTEXT* plain = Alloc_plaintext();
  Encode_val_at_level(plain, eval->_encoder, const_val, Get_ciph_level(ciph), 1,
                      false);
...
CIPHERTEXT* Fast_switch_key(CIPHERTEXT* res, SWITCH_KEY* key,
  ...
  uint64_t rtm_bs_md = 0;
  if (Rtlib_in_bootstrap) rtm_bs_md = Mark_rtm_start();
  Reduce_rns_base(c0, ciph_c0, crt);
  Reduce_rns_base(c1, ciph_c1, crt);
  if (Rtlib_in_bootstrap) Mark_rtm_end(RTM_BS_MOD_DOWN, rtm_bs_md);
  Free_ciphertext(ciph);
  return res;
}

CIPHERTEXT* Fast_switch_key_ext_opt(CIPHERTEXT* res, SWITCH_KEY* key,
                                    CKKS_EVALUATOR* eval,
                                    VALUE_LIST* precomputed, bool output_ntt) {
  RTLIB_TM_START(RTM_FAST_SWITCH_KEY, rtm);
  ...
  POLYNOMIAL** key0 = (POLYNOMIAL**)malloc((sizeof(POLYNOMIAL*)) * part_size);
  POLYNOMIAL** key1 = (POLYNOMIAL**)malloc((sizeof(POLYNOMIAL*)) * part_size);
  POLYNOMIAL   poly0[part_size];
  POLYNOMIAL   poly1[part_size];
  for (uint32_t idx = 0; idx < part_size; idx++) {
    key0[idx] = &poly0[idx];
    key1[idx] = &poly1[idx];
    Derive_poly(&poly0[idx], Get_pk0(Get_swk_at(key, idx)), Get_poly_level(c0),
                Get_num_p(c0));
    Derive_poly(&poly1[idx], Get_pk1(Get_swk_at(key, idx)), Get_poly_level(c1),
                Get_num_p(c1));
  }
  uint64_t rtm_bs_dp = 0;
  if (Rtlib_in_bootstrap) rtm_bs_dp = Mark_rtm_start();
  Fast_dotprod_poly(c0, raised_c1, key0, Get_q_primes(crt), Get_p_primes(crt),
                    part_size);

  Fast_dotprod_poly(c1, raised_c1, key1, Get_q_primes(crt), Get_p_primes(crt),
                    part_size);
  if (Rtlib_in_bootstrap) Mark_rtm_end(RTM_BS_DOT_PROD, rtm_bs_dp);
  if (!output_ntt) {
  ...
  free(key0);
  free(key1);
  RTLIB_TM_END(RTM_FAST_SWITCH_KEY, rtm);
  return res;
}
...
CIPHERTEXT* Fast_switch_key_ext(CIPHERTEXT* res, SWITCH_KEY* key,
  ...
  if (Get_rtlib_config(CONF_FAST_DOT_PROD)) {
    return Fast_switch_key_ext_opt(res, key, eval, precomputed, output_ntt);
  }
  RTLIB_TM_START(RTM_FAST_SWITCH_KEY, rtm);
  size_t part_size = LIST_LEN(precomputed);
  IS_TRUE(part_size <= Get_swk_size(key), "unmatched size");
  ...
  POLYNOMIAL*  c0  = Get_c0(res);
  POLYNOMIAL*  c1  = Get_c1(res);
  uint64_t rtm_bs_dp2 = 0;
  if (Rtlib_in_bootstrap) rtm_bs_dp2 = Mark_rtm_start();
  ...
  if (Rtlib_in_bootstrap) Mark_rtm_end(RTM_BS_DOT_PROD, rtm_bs_dp2);
  if (!output_ntt) {
    Conv_ntt2poly_inplace(c0, crt);
    Conv_ntt2poly_inplace(c1, crt);
  }
