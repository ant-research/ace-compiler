...
typedef struct {
  ...
  VL_VL_I64*  _m_mod_nb;           // {Q % new_base_j}
  VL_VL_I64*  _m_inv_mod_nb;       // {Q^-1 % new_base_j}
  VL_VL_I64*  _m_inv_mod_nb_prec;  // precomputed const for  {Q^-1 % new_base_j}
  VL_BIGINT*  _hat;                // BIG_INT, Q/q_i
  VALUE_LIST* _hat_mod_nb;         // {{Q/q_j} % new_base_j}
                                   // For Q -> VL_VL_VL_I64
                                   // For P -> VL_VL_I64
  VL_BIGINT* _hat_inv;             // BIG_INT, mod inverse of hat, (Q/q_i)^-1
  VL_VL_I64* _hat_inv_mod_self;    // {((Q/q_i)^-1)%q_i}
                                   // For Q -> VL_VL_I64
                                   // For P -> VL_I64
...
static inline VL_I64* Get_pinvmodq_prec(CRT_PRIMES* crt_primes) {
  IS_TRUE(Is_p(crt_primes), "invliad type");
