...
#if 0
static inline SWITCH_KEY* Inv_switch_key(SWITCH_KEY* swk) {
  SWITCH_KEY *inv = Alloc_switch_key();
  VALUE_LIST* pub_keys = swk->_public_keys;
  POLYNOMIAL* pk0 = Get_pk0(Get_swk_at(swk, 0));

  uint32_t rd = Get_rdgree(pk0);
  uint32_t num_q = Get_num_q(pk0);
  uint32_t num_p = get_num_p(pk0);
  uint32_t num_parts = LIST_LEN(pub_keys);
  Init_switch_key(inv, rd, num_q, num_p, num_parts);
  FOR_ALL_ELEM(pub_keys, idx) {
    PUBLIC_KEY *pk_idx = (PUBLIC_KEY*)PTR_VALUE_AT(pub_keys, idx);
    POLYNOMIAL *key0 = Get_pk0(pk_idx);
    POLYNOMIAL *key1 = Get_pk1(pk_idx);
