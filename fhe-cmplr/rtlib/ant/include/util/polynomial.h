...
static inline size_t Get_decomp_idx(size_t num_q, CRT_CONTEXT* crt) {
  CRT_PRIMES* q_parts   = Get_qpart(crt);
  size_t      part_size = Get_per_part_size(q_parts);
  return (size_t)floor(num_q / part_size);
}

static inline size_t Get_num_decomp(size_t num_q, CRT_CONTEXT* crt) {
  CRT_PRIMES* q_parts    = Get_qpart(crt);
  size_t      part_size  = Get_per_part_size(q_parts);
...
static inline size_t Get_num_decomp_poly(POLYNOMIAL* poly, CRT_CONTEXT* crt) {
  return Get_num_decomp(Get_num_q(poly), crt);
}
...
static inline int64_t* Get_poly_coeffs_at(POLYNOMIAL* poly, uint32_t idx) {
  ...
  IS_TRUE(idx < Get_num_alloc_primes(poly), "idx outof bound");
...
static inline void Copy_low_level_polynomial(POLYNOMIAL* dest,
  ...
  FMT_ASSERT(Get_poly_level(src) >= Get_poly_level(dest), "level out of range");
  FMT_ASSERT(Get_num_p(dest) >= Get_num_p(src), "unmatched p primes cnt");
...
void Multiply_add3(POLYNOMIAL* res, POLYNOMIAL** p0, POLYNOMIAL** p1,
                   VALUE_LIST* q_primes, VALUE_LIST* p_primes);
...
//! @brief precompute for fast multiply
...
//! @brief Reduce and rescale Polynomial
