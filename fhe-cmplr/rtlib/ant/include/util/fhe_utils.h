...
static inline int64_t Sub_int64_with_mod(int64_t val1, int64_t val2,
  ...
  // Branchless: add modulus when result is negative.
  // (res < 0) yields 0 or 1; -(int64_t)(1) yields -1 (all bits set);
  // (modulus & -1) == modulus, (modulus & 0) == 0.
  res += (modulus & (-(int64_t)(res < 0)));
  return res;
}
...
/**
 * @brief Barrett Modular Reduction
 * reauire: 0 <= val < mod_val^2
 *
 * @param val value to be mod
 * @param modulus modulus for Barrett Modular Reduction
 * @return int64_t
 */
static inline int64_t Mod_barrett_1282(UINT128_T val, MODULUS* modulus) {
  uint64_t mod_val = (uint64_t)Get_mod_val(modulus);
  IS_TRUE(val < (INT128_T)mod_val * mod_val, "val not supported for barret");
  int64_t m = (int64_t)Get_br_m(modulus);
  int64_t k = (int64_t)Get_br_k(modulus);
  // IS_TRUE(k >= 64, "invalid k");
  int64_t   val_h64 = (int64_t)(val >> 64);
  uint64_t  val_l64 = (uint64_t)(val);
  int64_t   res;
  UINT128_T tmp_low  = (UINT128_T)val_l64 * m;
  UINT128_T tmp_high = (UINT128_T)val_h64 * m;
  if (k >= 64) {
    tmp_low >>= 64;
    tmp_low += tmp_high;
    tmp_low >>= (k - 64);
  ...
  res = (uint64_t)(val - (tmp_low * mod_val));
  if (res >= mod_val) {
    return res - mod_val;
  } else {
    return res;
...
// res = (val1 - val2) * val3 % mod
static inline int64_t Sub_fast_mul_mod(uint64_t val1, uint64_t val2,
                                       uint64_t val3, uint64_t val3_inv,
                                       uint64_t mod) {
  int64_t sub_val = (int64_t)val1 - (int64_t)val2;
  if (sub_val < 0) sub_val += mod;
  uint64_t q      = (uint64_t)(((UINT128_T)sub_val * val3_inv) >> 64);
  uint64_t yprime = (sub_val * val3) - q * mod;
  uint64_t res    = yprime >= mod ? yprime - mod : yprime;
  IS_TRUE(res < mod, "invalid reduction");
  return (int64_t)res;
}
