...
//! @brief A list for automorphism index
typedef struct {
  uint32_t       _auto_idx;
  UT_hash_handle HH;
} AUTO_IDX_LIST;
...
static inline uint32_t Get_precomp_auto_idx(CKKS_KEY_GENERATOR* generator,
                                            int32_t             rot_idx) {
  PRECOMP_AUTO_IDX_MAP* result = NULL;
...
static inline void Insert_precomp_auto_order(CKKS_KEY_GENERATOR* generator,
  ...
  PRECOMP_AUTO_ORDER_MAP* map = NULL;
...
static inline VALUE_LIST* Get_precomp_auto_order(CKKS_KEY_GENERATOR* generator,
                                                 uint32_t            auto_idx) {
  PRECOMP_AUTO_ORDER_MAP* result = NULL;
...
//! @brief Clean up AUTO_IDX_LIST
static inline void Free_auto_idx_list(AUTO_IDX_LIST* auto_idx_list) {
  AUTO_IDX_LIST* current;
  AUTO_IDX_LIST* tmp;
  HASH_ITER(HH, auto_idx_list, current, tmp) {
    HASH_DEL(auto_idx_list, current);
    free(current);
  }
}
