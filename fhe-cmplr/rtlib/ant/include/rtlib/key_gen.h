...
static inline POLY* Swk_c0(bool is_rot, int32_t rot_idx) {
  SW_KEY swk = Swk(is_rot, rot_idx);

  VALUE_LIST* pub_keys  = swk->_public_keys;
  VALUE_LIST* poly_list = Alloc_value_list(PTR_TYPE, LIST_LEN(pub_keys));
  FOR_ALL_ELEM(pub_keys, idx) {
    Set_ptr_value(poly_list, idx, (PTR)Pk0_at(swk, idx));
  }
  return (POLY*)Get_ptr_values(poly_list);
}

static inline POLY* Swk_c1(bool is_rot, int32_t rot_idx) {
  SW_KEY      swk       = Swk(is_rot, rot_idx);
  VALUE_LIST* pub_keys  = swk->_public_keys;
  VALUE_LIST* poly_list = Alloc_value_list(PTR_TYPE, LIST_LEN(pub_keys));
  FOR_ALL_ELEM(pub_keys, idx) {
    Set_ptr_value(poly_list, idx, (PTR)Pk1_at(swk, idx));
  }
  return (POLY*)Get_ptr_values(poly_list);
}
