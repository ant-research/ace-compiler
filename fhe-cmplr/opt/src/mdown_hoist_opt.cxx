...
public:
  ...
  RETV Pre_handle_bb(VISITOR* visitor, BB_PTR bb) {
    if (bb->Is_phi()) {
      ...
      HSSA_CONTAINER& cont = Cfg().Hssa_cont();
  ...
  template <typename RETV, typename VISITOR>
  RETV Post_handle_cr(VISITOR* visitor, HCR_PTR expr) {
    if (expr->Kind() == CK_OP) {
      OP_HCR_DATA_PTR op_cr      = expr->Cast_to_op_cr();
      uint32_t        kid_md_cnt = 0;
      MDOWN_FLAG      f          = NONE;
      for (size_t idx = 0; idx < op_cr->Kid_cnt(); idx++) {
        HCR_PTR kid = expr->Kid(idx);
        _opt_driver.Prop_flag(expr, kid);
        if (_opt_driver.Is_set_exp_flag(kid, IS_MDOWN) ||
            _opt_driver.Is_set_exp_flag(kid, KID_HAS_MDOWN)) {
          kid_md_cnt++;
        }
      }
      if (_opt_driver.Is_mdown_check_point(expr) &&
          kid_md_cnt == op_cr->Kid_cnt() &&
          !_opt_driver.Is_set_exp_flag(expr, IS_CAND) &&
          !_opt_driver.Is_set_exp_flag(expr, CANNOT_EXTEND)) {
        const NODE_INFO& parent = visitor->Parent(1);
        if (_opt_driver.Iteration() == 0 || Is_inplaced_add(expr, parent)) {
          _opt_driver.Set_exp_flag(expr, IS_CAND);
    ...
    if (Is_mdown(expr)) {
      _opt_driver.Set_exp_flag(expr, IS_MDOWN);
    }
    Set_cannot_extend(expr);
  }

  template <typename RETV, typename VISITOR>
  RETV Post_handle_sr(VISITOR* visitor, HSTMT_PTR stmt) {
    if (stmt->Kind() == SK_ASSIGN) {
      HCR_PTR rhs = stmt->Rhs();
      _opt_driver.Prop_flag(stmt->Lhs(), rhs);
    }
  }

  bool Is_inplaced_add(HCR_PTR expr, const NODE_INFO& parent) {
    HSTMT_PTR par_stmt = parent.second;
    if (parent.first != Null_ptr) return false;
    if (par_stmt->Kind() != SK_ASSIGN) return false;
    HCR_PTR         lhs      = par_stmt->Lhs();
    OP_HCR_DATA_PTR add_expr = expr->Cast_to_op_cr();
  ...
  void Set_cannot_extend(HCR_PTR expr) {
    // if moddown used in precompute, mark kid0 canot extend
    if (Is_precomp(expr) &&
        (_opt_driver.Is_set_exp_flag(expr->Kid(0), KID_HAS_MDOWN) ||
         _opt_driver.Is_set_exp_flag(expr->Kid(0), IS_MDOWN))) {
      _opt_driver.Set_exp_flag(expr->Kid(0), CANNOT_EXTEND);
      return;
    }
    ...
    if (expr->Opcode() ==
      ...
      const uint32_t* is_mul_ciph =
          expr->Attr<uint32_t>(_opt_driver._lower_ctx->Attr_name(
              fhe::core::FHE_ATTR_KIND::MUL_CIPH));
      if (is_mul_ciph != nullptr && *is_mul_ciph != 0) {
        _opt_driver.Set_exp_flag(expr, CANNOT_EXTEND);
        return;
      }
    }
    ...
    if (expr->Opcode() == OPC_RESCALE &&
        (_opt_driver.Is_set_exp_flag(expr->Kid(0), KID_HAS_MDOWN) ||
         _opt_driver.Is_set_exp_flag(expr->Kid(0), IS_MDOWN))) {
      _opt_driver.Clear_exp_flag(expr, KID_HAS_MDOWN);
    }
  }

  bool Is_mdown(HCR_PTR expr) {
    if (expr->Opcode() ==
        air::base::OPCODE(POLYNOMIAL_DID, fhe::poly::OPCODE::MOD_DOWN)) {
      return true;
    }
...
public:
  MD_COMMUTE_CTX(MDOWN_HOIST_OPT& opt_driver, HCR_PTR expr)
  ...
  template <typename RETV, typename VISITOR>
  RETV Handle_cr(VISITOR* visitor, HCR_PTR node) {
    ...
    if (node == _root_expr && _root_kid_idx != -1) {
      _opt_driver.Trace(TRACE_MD_FLOW, "Root node already visted\n");
      return node;
    }
    ...
    if (node == _root_expr) {
      ...
      for (auto md : _md_kids) {
        HCR_PTR kid = _root_expr->Kid(idx);
        if (kid != Null_ptr) {
    ...
    _opt_driver.Trace(TRACE_MD_FLOW, "\nAfter commutation: cr",
                      node->Id().Value(), "\n");
    _opt_driver.Trace_obj(TRACE_MD_FLOW, retv);
    return retv;
  }

  template <typename RETV, typename VISITOR>
  RETV Pre_handle_sr(VISITOR* visitor, HSTMT_PTR stmt) {
    _opt_driver.Trace(TRACE_MD_FLOW, "cr", stmt->Lhs()->Id().Value(),
                      "--def--> cr", stmt->Rhs()->Id().Value(), "\n");
    return RETV();
  }
  ...
  RETV Post_handle_sr(VISITOR* visitor, HSTMT_PTR stmt) {
    ...
    if (stmt->Kind() == SK_ASSIGN &&
      ...
      HCR_PTR ext_var = _opt_driver.Find_or_new_ext_var_with_ver(stmt->Lhs());
      if ((_opt_driver.Is_modswitch(stmt->Rhs()) ||
        ...
        HCR_PTR ext_op           = _opt_driver.Hssa_cont()->New_op_cr(
        ...
        ext_var->Cast_to_var_cr()->Set_def_stmt(ext_assign->Id());
        ...
        ext_var->Cast_to_var_cr()->Set_def_stmt(stmt->Id());
    ...
    _opt_driver.Trace(TRACE_MD_FLOW, "After commutation sr: \n");
    _opt_driver.Trace_obj(TRACE_MD_FLOW, stmt);
    return RETV();
  }

  template <typename RETV, typename VISITOR>
  RETV Handle_var(VISITOR* visitor, HCR_PTR expr) {
    ...
    if (cross_loop) {
      _opt_driver.Trace(TRACE_MD_FLOW,
                        "Cross loop, no need to follow phi UD\n");
    ...
    if (_opt_driver.Need_extend(expr)) {
      if (expr->Cast_to_var_cr()->Def_by_phi()) {
        if (cross_loop) {  // && !expr->Is_extend()
          HCR_PTR ext_var = _opt_driver.Find_or_new_ext_var_with_ver(expr);
          if (ext_var->Def_stmt() == Null_ptr &&
            ...
            if (def_bb->Loop_info()->Parent_id() == loop_info->Id()) {
              def_bb->Loop_info()->Exit_bb()->Prepend_stmt(ext_assign);
              _opt_driver.Trace(TRACE_MD_FLOW, "Gen extend stmt in BB",
                                def_bb->Loop_info()->Exit_bb()->Id().Value(),
                                "\n");
            } else {
              loop_info->Init_bb()->Append_stmt(ext_assign);
              _opt_driver.Trace(TRACE_MD_FLOW, "Gen extend stmt in BB",
                                loop_info->Init_bb()->Id().Value(), "\n");
            }
            ext_var->Cast_to_var_cr()->Set_def_stmt(ext_assign->Id());
          }
        }
        ...
        _opt_driver.Trace(TRACE_MD_FLOW, "Gen extend op:");
        _opt_driver.Trace_obj(TRACE_MD_FLOW, ext_op);
      } else if (expr->Cast_to_var_cr()->Def_by_chi()) {
        ...
        if (ext_var->Def_stmt() == Null_ptr) {
          ...
          _opt_driver.Trace(TRACE_MD_FLOW, "Gen extend stmt after chi\n");
          _opt_driver.Trace_obj(TRACE_MD_FLOW, ext_assign);
          ext_var->Cast_to_var_cr()->Set_def_stmt(ext_assign->Id());
        }
        ret = ext_var;
      } else {
        ...
        _opt_driver.Trace(TRACE_MD_FLOW, "New/Find ext preg for cr",
                          expr->Id().Value(), "->");
        _opt_driver.Trace_obj(TRACE_MD_FLOW, ext_var);
        ret = ext_var;
  ...
  bool Is_cross_loop(HCR_PTR expr, HPHI_PTR& last_phi) {
    if (expr->Kind() != CK_VAR) return false;
  ...
  template <typename RETV, typename VISITOR>
  RETV Handle_phi(VISITOR* visitor, HCR_PTR expr) {
    HPHI_PTR phi = expr->Def_phi();
    AIR_ASSERT(phi != Null_ptr);
    _opt_driver.Trace(TRACE_MD_FLOW, "!!!Enter phi bb", phi->Bb_id().Value(),
                      "\n");
    HPHI_PTR ext_phi = _opt_driver.Find_ext_phi(phi);
    if (ext_phi == Null_ptr) {
      ext_phi         = _opt_driver.New_ext_phi(phi);
      HCR_PTR ext_res = _opt_driver.Find_or_new_ext_var_with_ver(phi->Result());
      ext_phi->Set_result(ext_res->Id());
    }
    for (uint32_t idx = 0; idx < phi->Size(); idx++) {
      HCR_PTR opnd     = phi->Opnd(idx);
      HCR_PTR ext_opnd = _opt_driver.Find_or_new_ext_var_with_ver(opnd);
      ext_phi->Set_opnd(idx, ext_opnd->Id());
    }

    HCR_PTR ext_expr = _opt_driver.Find_or_new_ext_var_with_ver(expr);
    ext_expr->Set_defphi(ext_phi);
    _opt_driver.Trace(TRACE_MD_FLOW, "Create hphi:\n");
    _opt_driver.Trace_obj(TRACE_MD_FLOW, phi);
    _opt_driver.Trace(TRACE_MD_FLOW, "\n");
    _opt_driver.Trace_obj(TRACE_MD_FLOW, ext_phi);
  ...
  RETV Handle_op(VISITOR* visitor, HCR_PTR expr) {
    ...
    if (_opt_driver.Is_set_exp_flag(expr, IS_MDOWN)) {
      ...
      for (size_t idx = 0; idx < op_cr->Kid_cnt(); idx++) {
        HCR_PTR kid = expr->Kid(idx);
        _opt_driver.Trace(TRACE_MD_FLOW, "cr", expr->Id().Value(), "--");
        // expr->Print_opcode(expr->Opcode(), std::cout);
        _opt_driver.Trace(TRACE_MD_FLOW, "-->cr", kid->Id().Value(), "\n");
...
private:
  ...
  std::vector<HCR_PTR> _md_kids;
};

void MDOWN_HOIST_OPT::Run(HSSA_FUNC* func) {
  Set_cur_func(func);
  Trace(TRACE_IR_BEFORE_MD, "\n=========XIR before MODDOWN======\n");
  Trace_obj(TRACE_IR_BEFORE_MD, &Cfg());
  ...
  for (auto item : _worklist2) {
    // if (idx < 1)
    {
      Trace(TRACE_MD_FLOW, "Processing candidate ", idx, ":\n");
      if (Has_ext_attr(item)) {
        Trace(TRACE_MD_FLOW, "Already processed, skipping\n");
        idx++;
        continue;
      }
  ...
  Trace(TRACE_MD_STATICS, "Statictics: Moddown IR reduce cnt:", _factor_cnt,
        " Sink cnt:", _sink_cnt);
  Trace(TRACE_IR_AFTER_MD, "\n=========XIR before MODDOWN======\n");
  Trace_obj(TRACE_IR_AFTER_MD, &Cfg());
}

bool MDOWN_HOIST_OPT::Has_ext_attr(HCR_PTR expr) {
  bool            is_ext     = false;
  const uint32_t* is_ext_ptr = expr->Attr<uint32_t>(
      _lower_ctx->Attr_name(fhe::core::FHE_ATTR_KIND::EXTENDED));
...
void MDOWN_HOIST_OPT::Add_cand(air::opt::HCR_PTR expr) {
...
void MDOWN_HOIST_OPT::Create_worklist() {
  ...
  if (_config.Is_trace(TRACE_MD_FLOW)) {
    Print_worklist(_driver_ctx->Tstream());
  }
}
void MDOWN_HOIST_OPT::Mdown_commutation(HCR_PTR cand) {
  MD_COMMUTE_CTX               ctx(*this, cand);
  HSSA_UD_TRAV<MD_COMMUTE_CTX> commutor(ctx);
  commutor.Start<HCR_PTR>(cand);
#if 0
  // processing new phis
  ...
  for (it = _ext_phi_map.begin(); it != _ext_phi_map.end(); it++) {
    ...
    HCR_PTR  old_res = Hssa_cont()->Cr_ptr(old_phi->Result_id());
    HCR_PTR  new_res = Hssa_cont()->Cr_ptr(new_phi->Result_id());
    BB_PTR   phi_bb  = Cfg().Bb_ptr(new_phi->Bb_id());
    BB_PTR   idom    = Cfg().Bb_ptr(Cfg().Dom_info().Get_imm_dom(phi_bb));

    HCR_PTR ext_op =
        Hssa_cont()->New_op_cr(opcode, poly_type, poly_type, phi_bb->Spos());
...
void MDOWN_HOIST_OPT::Mark_extend(HCR_PTR op) {
  uint32_t is_extended = 1;
  op->Set_attr(_lower_ctx->Attr_name(core::FHE_ATTR_KIND::EXTENDED),
               &is_extended, 1);
  if (Is_encode(op)) {
    HCR_PTR        is_ext      = Hssa_cont()->New_cst_cr(1);
    const uint32_t ext_kid_idx = 4;
    op->Cast_to_op_cr()->Set_kid(ext_kid_idx, is_ext->Id());
  }
}

void MDOWN_HOIST_OPT::Extend_op(HCR_PTR op) {
  air::base::OPCODE opcode =
      air::base::OPCODE(POLYNOMIAL_DID, fhe::poly::OPCODE::EXTEND);
  TYPE_ID poly_type = _lower_ctx->Get_poly_type_id();
  if (op->Kind() == CK_OP) {
    for (uint32_t idx = 0; idx < op->Kid_cnt(); idx++) {
      HCR_PTR kid = op->Kid(idx);
      if (Need_extend(kid)) {
        HCR_PTR ext_op =
            Hssa_cont()->New_op_cr(opcode, poly_type, poly_type, op->Spos());
        ext_op->Set_kid(0, kid);
        op->Set_kid(idx, ext_op->Id());
      }
    }
    Mark_extend(op);
  } else if (op->Kind() == CK_VAR && Need_extend(op)) {
    HSTMT_PTR def_stmt = op->Def_stmt();
    AIR_ASSERT(def_stmt != Null_ptr);
    HCR_PTR ext_var = Hssa_cont()->New_preg_cr(op);
    def_stmt->Set_lhs(ext_var->Id());
    _ext_expr_map[op->Id().Value()] = ext_var;
  }
}

bool MDOWN_HOIST_OPT::Need_extend(HCR_PTR op) {
...
void MDOWN_HOIST_OPT::Mdown_factor(HCR_PTR cand, NODE_INFO& cand_parent) {
  // step1 : remove modown from cand's kids
  for (uint32_t idx = 0; idx < cand->Kid_cnt(); idx++) {
    HCR_PTR kid = cand->Kid(idx);
    if (Is_mdown(kid)) {
  ...
  TYPE_ID poly_type = _lower_ctx->Get_poly_type_id();
  HCR_PTR new_md_op =
      Hssa_cont()->New_op_cr(md_opcode, poly_type, poly_type, cand->Spos());
  Set_exp_flag(new_md_op, IS_MDOWN);
  ...
  if (cand_parent.first == Null_ptr) {
    ...
    HCR_PTR   lhs        = par_stmt->Lhs();
    HCR_PTR   ext_lhs    = Find_or_new_ext_var_with_ver(lhs);
    HSTMT_PTR ext_assign = Hssa_cont()->New_assign_stmt(ext_lhs, cand);
    new_md_op->Set_kid(0, ext_lhs->Id());
    par_stmt->Cast_to_assign_sr()->Set_rhs(new_md_op->Id());
    Cfg().Bb_ptr(par_stmt->Bb_id())->Insert_stmt_before(ext_assign, par_stmt);
    lhs->Cast_to_var_cr()->Set_def_stmt(par_stmt->Id());
    Trace(TRACE_MD_FLOW, "After factoring\n");
    Trace_obj(TRACE_MD_FLOW, ext_assign);
    Trace_obj(TRACE_MD_FLOW, par_stmt);
  } else {
    HCR_PTR  par_cr   = cand_parent.first;
    uint32_t cand_idx = par_cr->Kid_idx(cand);
    par_cr->Set_kid(cand_idx, new_md_op->Id());
    Trace(TRACE_MD_FLOW, "After factoring\n");
    Trace_obj(TRACE_MD_FLOW, par_cr);
  }
  _factor_cnt++;
}

HCR_PTR MDOWN_HOIST_OPT::New_ext_var(HCR_PTR var_expr) {
  FUNC_SCOPE* fscope    = Hssa_cont()->Air_cont()->Parent_func_scope();
  TYPE_PTR    poly_type = _lower_ctx->Get_poly_type(&fscope->Glob_scope());
  std::string var_name  = var_expr->Cast_to_var_cr()->Name(Hssa_cont());
  std::string ext_str(var_name);
  ext_str.append(".ext");
  ADDR_DATUM_PTR ext_var =
      fscope->New_var(poly_type, ext_str.c_str(), var_expr->Spos());
  HCR_PTR ext_expr = Hssa_cont()->New_var_cr(ext_var);
  return ext_expr;
}

bool MDOWN_HOIST_OPT::Has_ext_var(HCR_PTR var_expr) {
  std::map<uint32_t, HCR_PTR>::iterator it =
...
HCR_PTR MDOWN_HOIST_OPT::Find_or_new_ext_var(HCR_PTR var_expr) {
  uint32_t var_id  = var_expr->Cast_to_var_cr()->Var_id();
  uint32_t sub_idx = var_expr->Cast_to_var_cr()->Sub_idx();
  uint32_t is_preg =
      var_expr->Cast_to_var_cr()->Var_kind() == air::opt::VAR_KIND::PREG ? 1
                                                                         : 0;
  ...
  if (it == _ext_sym_map.end()) {
    HCR_PTR ext_var   = New_ext_var(var_expr);
...
HCR_PTR MDOWN_HOIST_OPT::Find_or_new_ext_var_with_ver(HCR_PTR expr) {
  std::map<uint32_t, HCR_PTR>::iterator it =
      _ext_expr_map.find(expr->Id().Value());
  if (it != _ext_expr_map.end()) {
    return it->second;
  } else {
    HCR_PTR ext_var = Find_or_new_ext_var(expr);
    HCR_PTR ext_var_at_ver =
        Hssa_cont()->New_var_with_ver(ext_var, expr->Cast_to_var_cr()->Ver());
...
bool MDOWN_HOIST_OPT::Is_mdown(HCR_PTR expr) {
...
bool MDOWN_HOIST_OPT::Is_encode(HCR_PTR expr) {
...
bool MDOWN_HOIST_OPT::Is_modswitch(HCR_PTR expr) {
  if (expr->Opcode() ==
...
bool MDOWN_HOIST_OPT::Is_mdown_check_point(HCR_PTR expr) {
  if (expr->Opcode() !=
...
MDOWN_FLAG MDOWN_HOIST_OPT::Get_exp_flag(air::opt::HCR_PTR expr) {
  std::map<uint32_t, MDOWN_FLAG>::iterator it;
  it = _exp_flags.find(expr->Id().Value());
  if (it != _exp_flags.end())
    return it->second;
  else
    return NONE;
}

void MDOWN_HOIST_OPT::Set_exp_flag(air::opt::HCR_PTR expr, MDOWN_FLAG f) {
  std::map<uint32_t, MDOWN_FLAG>::iterator it;
  it = _exp_flags.find(expr->Id().Value());
  if (it != _exp_flags.end()) {
    MDOWN_FLAG old_f               = it->second;
    _exp_flags[expr->Id().Value()] = (MDOWN_FLAG)(old_f | f);
  } else {
    _exp_flags[expr->Id().Value()] = f;
  }
}

void MDOWN_HOIST_OPT::Clear_exp_flag(air::opt::HCR_PTR expr, MDOWN_FLAG f) {
  std::map<uint32_t, MDOWN_FLAG>::iterator it;
  it = _exp_flags.find(expr->Id().Value());
  if (it != _exp_flags.end()) {
    MDOWN_FLAG old_f               = it->second;
...
bool MDOWN_HOIST_OPT::Is_set_exp_flag(air::opt::HCR_PTR expr, MDOWN_FLAG f) {
  std::map<uint32_t, MDOWN_FLAG>::iterator it;
  it = _exp_flags.find(expr->Id().Value());
  if (it != _exp_flags.end()) {
...
void MDOWN_HOIST_OPT::Prop_flag(HCR_PTR expr1, HCR_PTR expr2) {
  if (!_lower_ctx->Is_poly_type(expr2->Rtype_id()) ||
      !_lower_ctx->Is_poly_type(expr1->Rtype_id()))
    return;
  MDOWN_FLAG old_f = Get_exp_flag(expr1);
  if (Is_set_exp_flag(expr2, IS_MDOWN) ||
      Is_set_exp_flag(expr2, KID_HAS_MDOWN)) {
    Set_exp_flag(expr1, KID_HAS_MDOWN);
  }

  if (Is_set_exp_flag(expr2, CANNOT_EXTEND)) {
    Set_exp_flag(expr1, CANNOT_EXTEND);
  }
  if (old_f != Get_exp_flag(expr1)) {
    Set_flag_changed(true);
  }
}

void MDOWN_HOIST_OPT::Mdown_sinking(HCR_PTR cand, NODE_INFO& cand_parent) {
  Trace(TRACE_MD_FLOW, "@@ Step 4: Sinking\n");
  if (cand_parent.first == Null_ptr) {
    HSTMT_PTR     par_stmt  = cand_parent.second;
    ...
    if (loop_info != Null_ptr) {
      if (bb->Kind() == BB_LOOP_BODY) {
        ...
        if (!Is_expr_used_in_loop(lhs, loop_info)) {
          BB_PTR exit_bb = loop_info->Exit_bb();
          bb->Remove_stmt(par_stmt);
          exit_bb->Prepend_stmt(par_stmt);
          Trace(TRACE_MD_FLOW, "  move stmt to loop exit: \n");
          Trace_obj(TRACE_MD_FLOW, par_stmt);
          _sink_cnt++;
        }
...
bool MDOWN_HOIST_OPT::Is_expr_used_in_loop(HCR_PTR       expr,
  ...
  for (uint32_t idx = 0; idx < uses->size() && !is_used; idx++) {
    ...
    if (use.Is_stmt()) {
      ...
      HCR_PTR phi_res = Hssa_cont()->Cr_ptr(use_phi->Result_id());
