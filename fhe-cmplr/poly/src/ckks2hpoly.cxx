...
POLY_LOWER_RETV CKKS2HPOLY::Handle_add_ciph(POLY_LOWER_CTX& ctx, NODE_PTR node,
  ...
  NODE_PTR add_0 = ctx.Poly_gen().New_poly_add(
      opnd0_pair.Node1(), opnd1_pair.Node1(), node->Spos());
  NODE_PTR add_1 = ctx.Poly_gen().New_poly_add(
      opnd0_pair.Node2(), opnd1_pair.Node2(), node->Spos());

  return POLY_LOWER_RETV(opnd0_pair.Kind(), add_0, add_1);
}

POLY_LOWER_RETV CKKS2HPOLY::Handle_add_plain(POLY_LOWER_CTX& ctx, NODE_PTR node,
                                             POLY_LOWER_RETV opnd0_pair,
                                             POLY_LOWER_RETV opnd1_pair) {
  CONTAINER* cntr = ctx.Poly_gen().Container();
  CMPLR_ASSERT((!opnd0_pair.Is_null() && !opnd1_pair.Is_null()), "null node");
...
POLY_LOWER_RETV CKKS2HPOLY::Handle_add_float(POLY_LOWER_CTX& ctx, NODE_PTR node,
                                             POLY_LOWER_RETV opnd0_pair,
                                             POLY_LOWER_RETV opnd1_pair) {
  ...
  // 1. encode float data
  NODE_PTR n_encode = Gen_encode_float_from_ciph(ctx, node, v_child0,
                                                 opnd1_pair.Node1(), false);
  ...
  NODE_PTR add_0 =
      ctx.Poly_gen().New_poly_add(opnd0_pair.Node1(), n_plain, node->Spos());
  NODE_PTR add_1 = cntr->Clone_node_tree(opnd0_pair.Node2());
  return POLY_LOWER_RETV(opnd0_pair.Kind(), add_0, add_1);
}
...
POLY_LOWER_RETV CKKS2HPOLY::Expand_relin(POLY_LOWER_CTX& ctx, NODE_PTR node,
  ...
  NODE_PTR n_num_part =
      ctx.Poly_gen().New_poly_node(NUM_DECOMP, ui32_type, spos);
  ...
  NODE_PTR n_prod_c0 = ctx.Poly_gen().New_dot_prod(
      ctx.Poly_gen().New_var_load(v_precomp, spos), n_swk_c0, n_num_part, spos);
  NODE_PTR n_prod_c1 = ctx.Poly_gen().New_dot_prod(
      ctx.Poly_gen().New_var_load(v_precomp, spos), n_swk_c1,
...
POLY_LOWER_RETV CKKS2HPOLY::Expand_rotate(POLY_LOWER_CTX& ctx, NODE_PTR node,
  ...
  // generate keyswitch dot product v_prod = DOT_PROD(v_)
  NODE_PTR n_prod_c0 = ctx.Poly_gen().New_dot_prod(
      ctx.Poly_gen().New_var_load(v_precomp, spos), n_swk_c0, n_num_part, spos);
  NODE_PTR n_prod_c1 = ctx.Poly_gen().New_dot_prod(
      ctx.Poly_gen().New_var_load(v_precomp, spos), n_swk_c1,
...
NODE_PTR CKKS2HPOLY::Gen_encode_float_from_ciph(POLY_LOWER_CTX& ctx,
  ...
  return ctx.Poly_gen().New_encode(n_cst, n_len, n_scale, n_level, n_is_ext,
                                   spos);
}
