...
PREG_PTR Gen_call_rotate(CKKS_IR_GEN& ir_gen, const char* fname,
  ...
  n_rotate->Set_attr("nums", &rot_idx, 1);
  ...
  ir_gen.Analyze_ckks_params(fs);

  // back to main_graph
  CONTAINER*  main_cntr   = ir_gen.Container();
  ...
  s_call->Node()->Set_attr(
      ir_gen.Lower_ctx().Attr_name(FHE_ATTR_KIND::MUL_DEPTH), &depth, 1);
