...
STMT_PTR TENSOR2SIHE_IMPL::Gen_call_approx_relu(VECTOR2SIHE_CTX& ctx,
  ...
  NODE_PTR opnd1 = cntr->New_ldp(arg, spos);
  ...
  if (value_range > 1.0) {
    ...
    CONSTANT_PTR vr_cst   = glob_scope->New_const(
        CONSTANT_KIND::FLOAT, f32_type, (long double)(1. / value_range));
    NODE_PTR ld_cst = cntr->New_ldc(vr_cst, spos);
    opnd1           = SIHE_GEN(cntr, &lower_ctx).Gen_mul(opnd1, ld_cst, spos);
  }
