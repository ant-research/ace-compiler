...
public:
  ...
  SECURITY_LEVEL Security_level() const { return static_cast<SECURITY_LEVEL>(_security_level); }
  ...
  // _security_level changed to uint64_t to match K_UINT64 option descriptor type.
  // The option parser writes 8 bytes via *((uint64_t*)field) = value, so
  // uint16_t fields get their upper 6 bytes spilled into adjacent fields.
  uint64_t       _security_level            = HE_STD_NOT_SET;
