...
public:
  ...
  uint32_t Input_level() {
    if (Func_scope()->Owning_func()->Entry_point()->Is_program_entry()) {
      uint32_t input_lev_config = _config->Input_lev();
...
RETV CORE_ANA_IMPL::Handle_idname(VISITOR*            visitor,
  ...
  uint32_t opnd_lev  = ana_ctx.Input_level() + 1;
  if (mul_level == opnd_lev || opnd_lev == 1) {
    return RETV{false, mul_level};
  }

  ckks::CKKS_GEN ckks_gen(formal->Container(), ana_ctx.Lower_ctx());
  CONTAINER*     cntr      = ssa_cntr->Container();
  NODE_PTR       modswitch = cntr->New_ld(formal->Addr_datum(), formal->Spos());
  ana_ctx.Set_node_mul_level(modswitch, opnd_lev);
  while (mul_level < opnd_lev) {
    modswitch = ckks_gen.Gen_modswitch(modswitch);
    --opnd_lev;
    ana_ctx.Set_node_mul_level(modswitch, opnd_lev);
  }
  STMT_PTR st = cntr->New_st(modswitch, formal->Addr_datum(), formal->Spos());
  ana_ctx.Trace(ckks::TRACE_CKKS_TRAN_RES, "Lower level of input ciphertext:");
  ana_ctx.Trace_obj(ckks::TRACE_CKKS_TRAN_RES, st);
  cntr->Stmt_list().Prepend(st);
  return RETV{false, mul_level};
}
...
RETV CORE_ANA_IMPL::Handle_call(VISITOR* visitor, NODE_PTR call_node) {
  ...
  for (uint32_t id = 0; id < child_num; ++id) {
    ...
    if (child->Opcode() == ckks::OPC_RESCALE) {
      param = child->Child(0);
    }
    const char* level_name =
        ana_ctx.Lower_ctx()->Attr_name(FHE_ATTR_KIND::LEVEL);
    const uint32_t* param_level_ptr = param->Attr<uint32_t>(level_name);
...
RETV CKKS_ANA_IMPL::Handle_bootstrap(VISITOR* visitor, NODE_PTR bootstrap) {
  ...
  const char* attr_name =
      ana_ctx.Lower_ctx()->Attr_name(FHE_ATTR_KIND::RESCALE_LEVEL);
  const uint32_t* rescale_lev_attr = child->Attr<uint32_t>(attr_name);
  ...
  if (rescale_lev + mul_level <= ana_ctx.Input_level()) {
    ...
    for (uint32_t id = 0; id < parent_node->Num_child(); ++id) {
      ...
      ana_ctx.Trace(ckks::TRACE_CKKS_TRAN_RES, "Delete redundant bootstrap");
      ana_ctx.Trace_obj(ckks::TRACE_CKKS_TRAN_RES, ana_ctx.Parent_stmt());
    ...
    AIR_ASSERT_MSG(false, "not found child bootstrap node");
  }

  // set mul_depth attr for App_relu
  if (ana_ctx.Opt_bts_res_lev()) {
    ana_ctx.Set_node_mul_level(bootstrap, mul_level);
  }

  STMT_PTR parent_stmt = ana_ctx.Parent_stmt();
  ana_ctx.Trace_obj(ckks::TRACE_CKKS_ANA_RES, parent_stmt);
...
RETV CKKS_ANA_IMPL::Handle_encode(VISITOR* visitor, NODE_PTR encode) {
  ...
  if (!ana_ctx.Const_encode_level() || !is_entry_func) {
    return RETV{false, mul_level};
  }
