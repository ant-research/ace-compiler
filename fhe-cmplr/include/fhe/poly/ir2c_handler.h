...
public:
  ...
  template <typename RETV, typename VISITOR>
  void Handle_mod_down_fuse1(VISITOR* visitor, air::base::NODE_PTR node) {
    IR2C_CTX& ctx = visitor->Context();
    ctx << "Mod_down_fuse1(";
    air::base::NODE_PTR parent = ctx.Parent(1);
    if (parent != air::base::Null_ptr) {
  ...
  //! @brief Emit MULP_FAST to C code
  template <typename RETV, typename VISITOR>
  void Handle_prec_plain(VISITOR* visitor, air::base::NODE_PTR node) {
    // mul_fast only valuable when Ct_encode is on
    IR2C_CTX& ctx = visitor->Context();
    AIR_ASSERT(ctx.Ct_encode() && ctx.Ct_prec());
    ctx.template Emit_encode_precom<RETV, VISITOR>(visitor, node);
  }

  //! @brief Emit MULP_FAST to C code
  template <typename RETV, typename VISITOR>
  void Handle_mulp_fast(VISITOR* visitor, air::base::NODE_PTR node) {
    // mul_fast only valuable when Ct_encode is on
    IR2C_CTX& ctx = visitor->Context();
    AIR_ASSERT(ctx.Ct_encode() && ctx.Ct_prec());
