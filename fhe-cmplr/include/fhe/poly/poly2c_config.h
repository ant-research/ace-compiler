...
public:
  ...
  bool           Ct_prec() const { return _ct_prec; }
  size_t         Df_limit() const { return _df_limit; }
  ...
  bool        _ct_prec;    // compile time precompute
  bool        _free_poly;  // insert free_poly
  size_t      _df_limit;   // data file size limit in megabytes
...
#define DECLARE_POLY2C_CONFIG_ACCESS_API(cfg)                            \
  ...
  bool           Ct_prec() const { return cfg.Ct_prec(); }               \
  bool           Free_poly() const { return cfg.Free_poly(); }           \
  size_t         Df_limit() const { return cfg.Df_limit(); }             \
