...
class MFREE_UTIL {
public:
  MFREE_UTIL(air::base::STMT_PTR pos, const fhe::core::LOWER_CTX& lower_ctx)
      : _pos(pos), _lower_ctx(lower_ctx) {
...
private:
  ...
  void Gen_free_record(VAR_TYPE var) {
    ...
    if (_lower_ctx.Is_poly_type(var->Type_id())) {
      Append_free_stmt(Gen_ld(var));
    } else {
...
class MFREE_PASS {
public:
  MFREE_PASS(const fhe::core::LOWER_CTX&     ctx,
             const fhe::poly::POLY2C_CONFIG& cfg)
      : _lower_ctx(ctx), _cfg(cfg) {}
  ...
  bool Type_need_mfree(air::base::TYPE_PTR type) {
    if ((_lower_ctx.Is_plain_type(type->Id()) && !_cfg.Ct_encode()) ||
        _lower_ctx.Is_cipher_type(type->Id()) ||
        _lower_ctx.Is_cipher3_type(type->Id()) ||
        _lower_ctx.Is_poly_type(type->Id())) {
      return true;
...
private:
  ...
  const fhe::core::LOWER_CTX&     _lower_ctx;
  const fhe::poly::POLY2C_CONFIG& _cfg;
...
public:
  ...
  RETV Handle_node(VISITOR* visitor, air::base::NODE_PTR node) {
    if (node->Opcode() == air::core::OPC_ST &&
      ...
      MFREE_UTIL util(pos, _pass.Lower_ctx());
      util.Insert_mfree(retv);
    } else if (node->Is_st() &&
               node->Child(0)->Opcode() == fhe::poly::OPC_PRECOMP) {
      if (node->Has_sym()) {
        _pass.Mark_var_freed(node->Addr_datum());
        air::base::STMT_PTR pos = Find_insert_pos();
        AIR_ASSERT(pos != air::base::Null_ptr);
        MFREE_UTIL util(pos, _pass.Lower_ctx());
        util.Insert_plist_free(node->Addr_datum());
      } else if (node->Has_preg()) {
        _pass.Mark_var_freed(node->Preg());
        air::base::STMT_PTR pos = Find_insert_pos();
        AIR_ASSERT(pos != air::base::Null_ptr);
        MFREE_UTIL util(pos, _pass.Lower_ctx());
        util.Insert_plist_free(node->Preg());
      } else {
        AIR_ASSERT_MSG(false, "precomp result has no symbol");
      }
