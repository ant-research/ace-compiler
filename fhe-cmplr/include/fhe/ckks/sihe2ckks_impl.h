...
RETV SIHE2CKKS_IMPL::Handle_encode_in_bin_arith_node(VISITOR* visitor,
  ...
  // 5. gen is_ext node
  TYPE_PTR is_ext_type = cntr->Glob_scope()->Prim_type(PRIMITIVE_TYPE::INT_U32);
  NODE_PTR is_ext_node = cntr->New_intconst(is_ext_type, 0, spos);
  ...
  // copy ATTR
  const double* mask_len = encode_node->Attr<double>(nn::core::ATTR::MASK);
  if (mask_len != nullptr) {
    ckks_encode->Set_attr(nn::core::ATTR::MASK, mask_len, 1);
  }
  ckks_encode->Set_child(4, is_ext_node);
...
RETV SIHE2CKKS_IMPL::Handle_encode(VISITOR* visitor, NODE_PTR encode) {
  ...
  // copy ATTR
  const double* mask_len = encode->Attr<double>(nn::core::ATTR::MASK);
  if (mask_len != nullptr) {
    ckks_encode->Set_attr(nn::core::ATTR::MASK, mask_len, 1);
  }
  return RETV(ckks_encode);
