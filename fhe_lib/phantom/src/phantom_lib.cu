...
public:
    ...
    void Init_context_without_bootstrap()
    {
        ...
        {
            // Normalize rotation step to [0, slot_count)
            int normalized_step = rot;
            if (normalized_step < 0)
            {
                normalized_step = _slot_count + rot;
            }
            if (normalized_step >= _slot_count)
            {
                normalized_step = normalized_step % _slot_count;
            }
            if (find(gal_steps_vector.begin(), gal_steps_vector.end(), normalized_step) == gal_steps_vector.end())
                gal_steps_vector.push_back(normalized_step);
        }
