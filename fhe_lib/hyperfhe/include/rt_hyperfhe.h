...
#ifndef RTLIB_RT_HYPERFHE_RT_HYPERFHE_H
#define RTLIB_RT_HYPERFHE_RT_HYPERFHE_H

//! @brief rt_eval.h
//! Define common API for fhe-cmplr. Dispatch to specific implementations
//! built on top of HYPERFHE libraries.
...
inline void Encode_float(PLAIN plain, float* input, size_t len,
  ...
  Hyperfhe_encode_float(plain, input, len, sc_degree, level);
}

inline void Encode_float(PLAIN plain, float* input, size_t len,
                         SCALE_T sc_degree, int level) {
  Hyperfhe_encode_float_cst_lvl(plain, input, len, sc_degree, level);
}
...
inline void Copy_ciph(CIPHER res, CIPHER op) {
  if (res != op) {
    Hyperfhe_copy(res, op);
  }
}

inline void Zero_ciph(CIPHER res) { Hyperfhe_zero(res); }
...
SCALE_T Sc_degree(CIPHER ct) { return Hyperfhe_scale_degree(ct); }

SCALE_T Sc_factor(CIPHER ct) { return Hyperfhe_scale(ct); }

LEVEL_T Level(CIPHER ct) { return Hyperfhe_level(ct); }
