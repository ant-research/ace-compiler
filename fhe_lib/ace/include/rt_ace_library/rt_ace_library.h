...
#ifndef RTLIB_RT_ACE_LIBRARY_RT_ACE_LIBRARY_H
#define RTLIB_RT_ACE_LIBRARY_RT_ACE_LIBRARY_H

//! @brief rt_ace_library.h
//! Acelib-specific runtime wrapper.  Both libFHErt_ace.so and the
//! generated kernel .cu are compiled against the real SDK headers, so
//! cuckks::DeviceCipher / DevicePlain are the real types everywhere.
...
inline void Encode_float(PLAIN plain, float* input, size_t len,
  ...
  AceLibrary_encode_float(plain, input, len, sc_degree, level);
}

inline void Encode_float(PLAIN plain, float* input, size_t len,
                         SCALE_T sc_degree, int level) {
  AceLibrary_encode_float_cst_lvl(plain, input, len, sc_degree, level);
}
...
inline void Copy_ciph(CIPHER res, CIPHER op) {
  if (res != op) {
    AceLibrary_copy(res, op);
  }
}

inline void Zero_ciph(CIPHER res) { AceLibrary_zero(res); }

inline void Free_ciph(CIPHER res) { (void)res; }

inline void Free_plain(PLAIN res) { (void)res; }
...
SCALE_T Sc_degree(CIPHER ct) { return AceLibrary_scale_degree(ct); }

SCALE_T Sc_factor(CIPHER ct) { return AceLibrary_scale(ct); }

LEVEL_T Level(CIPHER ct) { return AceLibrary_level(ct); }
...
#endif  // RTLIB_RT_ACE_LIBRARY_RT_ACE_LIBRARY_H