...
public:
  ...
  Ciphertext Get_input_data(const char* name, size_t idx) {
    ...
    // Do NOT delete data — it is owned by _owned_io_ciphertexts.
    // Move the ciphertext out; the unique_ptr in _owned_io_ciphertexts
    // will release the moved-from object harmlessly on destruction.
  ...
  void Encode_float(Plaintext* pt, float* input, size_t len, SCALE_T scale,
    ...
    auto parm_id = _ctx->get_parms_id_from_level(level);
...
public:
  ...
  LEVEL_T Level(Ciphertext* op) {
    // Return the coefficient modulus count as the integer level,
    // consistent with phantom's LEVEL_T = size_t and fhe_cmplr expectations.
    return op->coeff_modulus_size();
  }
...
#if ACE_LIBRARY_ENABLE_BTS
void Make_bts_parms(
    CKKS_PARAMS* prog_param, frontend::HostParameter& parms_out,
    ...
    config::LTHoistingMode hoisting = config::LTHoistingMode::DOUBLE_HOIST)
{
    ...
    parms.set_coeff_modulus(
        frontend::CoeffModulus::Create(Flatten_modulus_blocks(modulus_blocks)));
#endif
...
private:
  ...
  static frontend::sec_level_type Security_level(
  ...
  void Configure_plain_parameters(CKKS_PARAMS* prog_param,
                                  frontend::HostParameter& parms,
                                  uint32_t degree,
                                  int n_special_primes);
  void Init_cpu_context(CKKS_PARAMS* prog_param,
                        const frontend::HostParameter& parms,
                        frontend::sec_level_type sec,
...
private:
  std::unique_ptr<frontend::HostContext> _cpu_ctx_owner;
  std::unique_ptr<frontend::HostContext> _host_context_owner;
  frontend::HostContext* _cpu_ctx = nullptr;
  frontend::HostContext* _host_context = nullptr;

  std::unique_ptr<frontend::KeyGenerator> _kgen;

  const frontend::SecretKey* _sk = nullptr;
  std::unique_ptr<frontend::PublicKey> _pk;
  std::unique_ptr<frontend::RelinKeys> _rlk;
  std::unique_ptr<frontend::GaloisKeys> _rtk;

  std::unique_ptr<frontend::CKKSEncoder> _encoder;
  std::unique_ptr<frontend::Encryptor> _encryptor;
  std::unique_ptr<frontend::Decryptor> _decryptor;

  cuckks::MEMORY_LAYOUT _data_layout;
  std::unique_ptr<cuckks::Evaluator> _gpu_eval_owner;
  cuckks::Evaluator* _gpu_eval = nullptr;
  cuckks::ContextPro* _ctx = nullptr;
  cuckks::Evaluator* _eval = nullptr;
#if ACE_LIBRARY_ENABLE_BTS
...
frontend::sec_level_type ACE_LIBRARY_CONTEXT::Security_level(
    const CKKS_PARAMS* prog_param, uint32_t degree) {
  ...
  if (degree < 4096 && sec != frontend::sec_level_type::none) {
    DEV_WARN("WARNING: degree %d too small, reset security level to none\n",
             degree);
    return frontend::sec_level_type::none;
  }
  return sec;
}
...
void ACE_LIBRARY_CONTEXT::Configure_plain_parameters(
    CKKS_PARAMS* prog_param, frontend::HostParameter& parms, uint32_t degree,
    int n_special_primes) {
...
void ACE_LIBRARY_CONTEXT::Init_cpu_context(
    CKKS_PARAMS* prog_param, const frontend::HostParameter& parms,
    frontend::sec_level_type sec, bool& use_sparse_wrapping) {
  if (Need_bts()) {
    Init_bts_cpu_context(prog_param, use_sparse_wrapping);
    return;
  }

  _cpu_ctx_owner = std::make_unique<frontend::HostContext>(parms, true, sec);
...
void ACE_LIBRARY_CONTEXT::Init_bts_cpu_context(CKKS_PARAMS* prog_param,
  ...
  _host_context_owner = std::make_unique<frontend::HostContext>(
      host_bts_parms, true, frontend::sec_level_type::none);
...
void ACE_LIBRARY_CONTEXT::Init_keys_and_codec(CKKS_PARAMS* prog_param,
                                              bool use_sparse_wrapping) {
  if (Need_bts()) {
    _kgen = std::make_unique<frontend::KeyGenerator>(*_cpu_ctx,
                                                     use_sparse_wrapping);
  } else {
    _kgen = std::make_unique<frontend::KeyGenerator>(*_cpu_ctx);
  }
  _sk   = &_kgen->secret_key();
  _pk = std::make_unique<frontend::PublicKey>();
  _kgen->create_public_key(*_pk);
  _rlk = std::make_unique<frontend::RelinKeys>();
  _kgen->create_relin_keys(*_rlk);
  _rtk = std::make_unique<frontend::GaloisKeys>();

  _encryptor = std::make_unique<frontend::Encryptor>(*_cpu_ctx, *_pk, *_sk);
  _decryptor = std::make_unique<frontend::Decryptor>(*_cpu_ctx, *_sk);
  _encoder = std::make_unique<frontend::CKKSEncoder>(*_cpu_ctx);
  _scaling_mod_size = prog_param->_scaling_mod_size;
  if (prog_param->_input_level > prog_param->_mul_depth) {
    DEV_WARN("WARNING: prog_param->_input_level > prog_param->_mul_depth\n");
  }
...
void ACE_LIBRARY_CONTEXT::Init_evaluator(bool use_sparse_wrapping) {
  ...
  _gpu_eval_owner = std::make_unique<cuckks::Evaluator>(
      cuckks::ContextPro::get_device_context(*_cpu_ctx), true);
  ...
  _gpu_eval->get_evk_from(*_rlk, cuckks::EvkConfigs::relin_key(_data_layout));
  _gpu_eval->get_evk_from(
      *_rtk, cuckks::EvkConfigs::galois_key(true, _data_layout));
...
void ACE_LIBRARY_CONTEXT::Init_bts_evaluator(bool use_sparse_wrapping) {
  _dev_eval_owner = std::make_unique<EvalWrapper>(
      *_encoder, cuckks::ContextPro::get_device_context(*_host_context), true);
...
#endif
#ifdef ACE_LIBRARY_32
  const auto evk_mode = frontend::EvkRequirementMode::PreferCompact;
#else
  const auto evk_mode = frontend::EvkRequirementMode::FullOnly;
#endif
  ...
  _dev_eval->get_evk_from(
      *_rtk, cuckks::EvkConfigs::galois_key(
                 true, _data_layout, cuckks::EvkTransferMode::FULL_TRANSFER,
                 std::nullopt,
                 evk_requirements.galois_keys.pre_galois_keys()));
  _dev_eval->get_evk_from(*_rlk,
                          cuckks::EvkConfigs::relin_key(_data_layout));
...
ACE_LIBRARY_CONTEXT::ACE_LIBRARY_CONTEXT() {
  ...
  CKKS_PARAMS* prog_param = Get_context_params();
  IS_TRUE(prog_param->_provider == LIB_ACE_LIBRARY, "provider is not AceLibrary");
  ...
  frontend::sec_level_type sec = Security_level(prog_param, degree);
  bool use_sparse_wrapping = false;
  _data_layout = cuckks::MEMORY_LAYOUT::ROW_MAJOR;
...
//! @brief Return an input ciphertext by name and index (by value).
//! The caller receives ownership of the returned ciphertext.
