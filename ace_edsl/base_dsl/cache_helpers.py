...
try:
    default_generated_ir_path = f"/tmp/{get_current_user()}/ace_edsl/"
except Exception as e:
    # If all else fails, provide a default fallback path
    default_generated_ir_path = "/tmp/ace_edsl/"
...
def check_func_name(jit_cache, func_name):
    if not func_name in jit_cache:
        jit_cache[func_name] = JitExecutor(None, None, None, None, None)
    return jit_cache
