...
class Executor:
    ...
    def for_dynamic(
        ...
        return self._loop_execute_range_dynamic(
            func, start, stop, step, used_args, iter_args, unroll, unroll_full
        )
    ...
    def for_constexpr(
        ...
        # Import Index here to avoid circular dependencies
        from .typing import Index
        for i in range(start, stop, step):
            log().debug("i  [%s] iter_args  [%s]", i, iter_args)
            # Wrap the loop variable in Index to enable MLIR constant generation
            index_wrapped = Index(i)
            log().debug("Wrapped %s in Index: %s (type: %s)", i, index_wrapped, type(index_wrapped))
            loop_results = func(index_wrapped, *used_args, *loop_results)
    ...
    def if_dynamic(
        ...
        return self._if_dynamic(pred, then_block, else_block, used_args, yield_args)
    ...
    def if_execute(
        ...
        # MLIR generation
        return self.if_dynamic(pred, then_block, else_block, used_args, yield_args)
...
def while_executor(
    ...
    return executor.while_execute(
        pred, while_before_block, while_after_block, used_args, yield_args, constexpr
    )
...
def if_executor(
    ...
    return executor.if_execute(
        pred, then_block, else_block, used_args, yield_args, constexpr
    )
...
# TODO: those should be moved to public API module and keep ast_helpers internal
...
def assert_executor(test, msg=None):
    ...
    fail = False
    if test and executor._is_dynamic_expression(test):
        # FIXME: for dynamic expression, we should use runtime assert directly
