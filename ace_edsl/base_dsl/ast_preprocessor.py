...
class DSLPreprocessor(ast.NodeTransformer):
    ...
    def exec(self, function_name, original_function, exec_globals):
        ...
        exec(self.code_object, exec_globals)
    ...
    def find_called_functions(self, tree):
        """Finds all functions called within a given AST."""
        ...
        class CallFinder(ast.NodeVisitor):
            ...
            def visit_Call(self, node):
                ...
                # Handle method calls like self.kernel()
        ...
        finder = CallFinder()
        finder.visit(tree)
        return finder.called_functions
    ...
    def transform_function(self, func_name, function_pointer, globals):
        """
        ...
        # Note: function_pointer should already be unwrapped by transform() method
        ...
        # Strip decorators from the function definition in the transformed AST.
        # The function is already wrapped by the DSL decorator at runtime, and
        # re-applying it during exec() can cause duplicate wrappers and IR.
        try:
        ...
        # Step 3. Find all called functions
        called_functions = self.find_called_functions(tree)

        # Step 4. Transform all called functions
        for func_name in called_functions:
            ...
            except DSLAstPreprocessorError as e:
                raise e
            except Exception as e:
                log().info(
                    "ASTPreprocessor skip transforming [%s], error [%s]", func_name, e
                )
    ...
    def transform(self, original_function, globals):
        """
        ...
        transformed_tree = self.transform_function(
            original_function.__name__, original_function, globals
        )
        unified_tree = ast.Module(body=transformed_tree, type_ignores=[])
        unified_tree = ast.fix_missing_locations(unified_tree)

        # Compile and return the transformed tree for the original function
        self.code_object = compile(unified_tree, filename=self.file_name, mode="exec")
        return unified_tree
            ...
            def visit_Call(self, node):
                ...
                if isinstance(node.func, ast.Name):
                    ...
                    if func_name in local_closure:
                        # TODO This is too restrictive. We can allow closures that do not mutate any
                        # variables, but it requires deeper analysis.
                        raise DSLAstPreprocessorError(
    ...
    # Decorators that enable preprocessing
    PREPROCESSABLE_DECORATORS = {
        # Core decorators
        "jit", "kernel",
        # Domain decorators (ace_edsl)
        "nn_kernel", "tensor_kernel", "vector_kernel", 
        "compute_kernel", "memory_kernel",
        "sihe_kernel", "ckks_kernel", "poly_kernel",
    }
    ...
    def check_decorator(self, node: ast.AST) -> bool:
        ...
        Accepts:
        - @jit, @kernel (core decorators)
        - @nn_kernel, @tensor_kernel, etc. (domain decorators)
        - @SomeDSL.jit (attribute access)
        """
        ...
        for d in decorator_list:
            if isinstance(d, ast.Call):
                ...
                if isinstance(d.func, ast.Attribute):
                    # @SomeDSL.jit(...) or @SomeDSL.kernel(...)
                    if d.func.attr in self.PREPROCESSABLE_DECORATORS:
                        if d.keywords == []:
                            return True
                        for keyword in d.keywords:
                            if keyword.arg == "preprocess":
                                try:
                                    ...
                                    pass
                        return True  # Default to True if no preprocess kwarg
                elif isinstance(d.func, ast.Name):
                    # @nn_kernel(...) or @tensor_kernel(...)
                    if d.func.id in self.PREPROCESSABLE_DECORATORS:
                        for keyword in d.keywords:
                            if keyword.arg == "preprocess":
                                try:
                                    if isinstance(keyword.value, ast.Constant):
                ...
                # @SomeDSL.jit (without call)
                if d.attr in self.PREPROCESSABLE_DECORATORS:
                    return True
            
            elif isinstance(d, ast.Name):
                # @nn_kernel (without call)
                if d.id in self.PREPROCESSABLE_DECORATORS:
                    return True
    ...
    def create_while_function(
        ...
        # TODO @zekunf: maybe need to self.visit & transform test_expr?
