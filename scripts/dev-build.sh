...
# Build type: default Release, override with CMAKE_BUILD_TYPE=Debug for debugging
...
if [ "$HELP" = true ]; then
    ...
    echo "  CMAKE_BUILD_TYPE      Build type (default: Release; set Debug for debugging)"
    echo ""
    echo "Examples:"
    echo "  ./scripts/dev-build.sh                         # Build and install (Release)"
    ...
    echo "  CMAKE_BUILD_TYPE=Debug ./scripts/dev-build.sh  # Debug build"
    exit 0
fi
...
if [ "$INSTALL_ONLY" = true ]; then
    ...
    for so_file in "$ACE_DIR/"*.so; do
        if [ -f "$so_file" ]; then
            cp -f "$so_file" "$PROJECT_ROOT/fhe_dsl/python/"
            echo -e "${GREEN}  Copied: $(basename $so_file)${NC}"
        fi
    done
    if [ -f "$ACE_DIR/lib/libFHErt_common.so" ]; then
        mkdir -p "$PROJECT_ROOT/fhe_dsl/python/lib/"
        cp -f "$ACE_DIR/lib/libFHErt_common.so" "$PROJECT_ROOT/fhe_dsl/python/lib/"
...
echo ""
echo "  2. Run tests:"
echo "     pytest tests/test_unit/frontend/ -v"
echo ""
