#!/bin/bash
# run_perf.sh — Run perf benchmarks across configs
#
# Usage:
#   ./run_perf.sh                          # all 6 configs
#   ./run_perf.sh -c base                 # single config
#   ./run_perf.sh -c base,lmr             # multiple configs
#   ./run_perf.sh -c all -m lenet.onnx    # single model
#   ./run_perf.sh --model-first           # iterate by model then config
#   ./run_perf.sh --compile-only          # compile only
#   ./run_perf.sh --list                  # list configs
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)
            ...
            echo "  -m MODEL        Single model name (e.g. lenet.onnx)"
