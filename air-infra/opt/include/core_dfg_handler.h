...
RETV CORE_DATA_FLOW_HANDLER::Handle_ist(VISITOR* visitor, NODE_PTR ist) {
  ...
  const SSA_CONTAINER*  ssa_cntr = ctx.Ssa_cntr();
  air::opt::CHI_NODE_ID chi_id   = ssa_cntr->Node_chi(ist->Id());
  SSA_VER_PTR  ssa_ver = ssa_cntr->Ver(ssa_cntr->Chi_node(chi_id)->Result_id());
...
RETV CORE_DATA_FLOW_HANDLER::Handle_ild(VISITOR* visitor, NODE_PTR ild) {
  ...
  DFG_BUILD_CTX&        ctx      = visitor->Context();
  const SSA_CONTAINER*  ssa_cntr = ctx.Ssa_cntr();
  air::opt::MU_NODE_ID  mu_id    = ssa_cntr->Node_mu(ild->Id());
  air::opt::MU_NODE_PTR mu       = ssa_cntr->Mu_node(mu_id);
  SSA_VER_PTR           ssa_ver  = ssa_cntr->Ver(mu->Opnd_id());
