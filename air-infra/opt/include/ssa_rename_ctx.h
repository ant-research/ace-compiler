...
public:
  ...
  SSA_VER_ID Gen_stmt_def(SSA_SYM_ID sym, air::base::STMT_ID stmt) {
    ...
    // add new version for sym child
    air::base::NODE_ID  node_id  = air::base::NODE_ID(stmt.Value());
    air::base::NODE_PTR node_ptr = _ssa_cont->Container()->Node(node_id);
    SSA_SYM_PTR         sym_ptr  = _ssa_cont->Sym(sym);
    SSA_SYM_ID          child_id = sym_ptr->Child_id();
    CHI_NODE_ID         list     = _ssa_cont->Has_chi(node_ptr)
                                       ? _ssa_cont->Node_chi(node_id)
                                       : CHI_NODE_ID();
    while (child_id != SSA_SYM_ID()) {
      CHI_NODE_PTR chi = _ssa_cont->New_chi(child_id);
      chi->Set_next(list);
      list     = chi->Id();
      child_id = _ssa_cont->Sym(child_id)->Sibling_id();
    }
    if (list != CHI_NODE_ID()) {
      _ssa_cont->Set_node_chi(node_id, list);
      Rename_chi_list(node_ptr);
    }
    return ver->Id();
  }
