...
private:
  ...
  SSA_SYM_PTR Get_ssa_sym(U64_SYM_MAP& map, SSA_SYM_KIND kind, uint32_t var,
                          uint32_t index, base::TYPE_ID type_id) {
    ...
    if (index != SSA_SYM::NO_INDEX) {
      air::base::TYPE_PTR ty_ptr = _ssa_cont->Glob_scope()->Type(type_id);
      ...
      SSA_SYM_PTR           p_sym = (p_it == map.end())
                                        ? _ssa_cont->New_sym(kind, var, p_idx, type_id)
  ...
  void Handle_variable(U64_SYM_MAP& map, SSA_SYM_KIND kind, uint32_t var,
    ...
    SSA_SYM_PTR sym = Get_ssa_sym(map, kind, var, index, type_id);
    if (def) {
...
public:
  void Handle_variable(air::base::ADDR_DATUM_ID var, uint32_t index,
                       base::TYPE_ID type_id, air::base::NODE_PTR node,
                       bool def) {
    SSA_SYM_KIND kind = SSA_SYM_KIND::ADDR_DATUM;
    Handle_variable(*_datum_map, kind, var.Value(), index, type_id, node, def);
  }
  ...
  void Handle_variable(air::base::PREG_ID var, uint32_t index,
    ...
    Handle_variable(*_preg_map, kind, var.Value(), index, type_id, node, def);
  }
  ...
  void Handle_virtual_variable(base::ADDR_DATUM_ID var, uint32_t index,
    ...
    SSA_SYM_KIND kind = SSA_SYM_KIND::ADDR_DATUM;
    SSA_SYM_PTR  sym =
        Get_ssa_sym(*_datum_map, kind, var.Value(), index, type_id);
    if (def) {
      AIR_ASSERT(node->Is_root());
      Append_phi_def(node->Stmt(), sym->Id());
      CHI_NODE_PTR chi        = _ssa_cont->New_chi(sym->Id());
      CHI_NODE_ID  pre_chi_id = _ssa_cont->Node_chi(node->Id());
      if (pre_chi_id != base::Null_id) {
        _ssa_cont->Chi_node(pre_chi_id)->Set_next(chi->Id());
      } else {
...
public:
  ...
  RETV Handle_ild(VISITOR* visitor, air::base::NODE_PTR node) {
    ...
    visitor->template Visit<RETV>(addr_child->Child(1));
    ...
    ctx.Handle_virtual_variable(array_addr_datum->Id(), SSA_SYM::NO_INDEX,
                                array_addr_datum->Type_id(), node, false);
  }

  template <typename RETV, typename VISITOR>
  RETV Handle_lda(VISITOR* visitor, air::base::NODE_PTR node) {
    // handle lda symbol
    base::ADDR_DATUM_PTR array_addr_datum = node->Addr_datum();
    base::ADDR_DATUM_ID array_addr_datum_id = node->Addr_datum_id();
    SIMPLE_BUILDER_CTX& ctx                 = visitor->Context();
    ctx.Handle_virtual_variable(array_addr_datum->Id(), SSA_SYM::NO_INDEX,
                                array_addr_datum->Type_id(), node, false);
  }
  ...
  RETV Handle_ldf(VISITOR* visitor, air::base::NODE_PTR node) {
    SIMPLE_BUILDER_CTX& ctx = visitor->Context();
    ctx.Handle_variable(node->Addr_datum_id(), node->Field_id().Value(),
                        node->Access_type_id(), node, false);
  }
  ...
  RETV Handle_ldp(VISITOR* visitor, air::base::NODE_PTR node) {
    visitor->Context().Handle_variable(node->Preg_id(), SSA_SYM::NO_INDEX,
                                       node->Rtype_id(), node, false);
  }

  template <typename RETV, typename VISITOR>
  RETV Handle_ldpf(VISITOR* visitor, air::base::NODE_PTR node) {
    visitor->Context().Handle_variable(node->Preg_id(),
                                       node->Field_id().Index(),
                                       node->Rtype_id(), node, false);
  }
  ...
  RETV Handle_st(VISITOR* visitor, air::base::NODE_PTR node) {
    ...
    visitor->Context().Handle_variable(addr_datum->Id(), SSA_SYM::NO_INDEX,
                                       addr_datum->Type_id(), node, true);
  }
  ...
  RETV Handle_ist(VISITOR* visitor, air::base::NODE_PTR node) {
    ...
    SIMPLE_BUILDER_CTX&  ctx              = visitor->Context();
    ctx.Handle_virtual_variable(array_addr_datum->Id(), SSA_SYM::NO_INDEX,
                                array_addr_datum->Type_id(), node, true);
  }
  ...
  RETV Handle_stf(VISITOR* visitor, air::base::NODE_PTR node) {
    visitor->template Visit<RETV>(node->Child(0));
    base::ADDR_DATUM_PTR addr_datum = node->Addr_datum();
    visitor->Context().Handle_variable(addr_datum->Id(),
                                       node->Field_id().Index(),
                                       node->Access_type_id(), node, true);
    visitor->Context().Handle_virtual_variable(
        addr_datum->Id(), SSA_SYM::NO_INDEX, addr_datum->Type_id(), node, true);
  }
  ...
  RETV Handle_stp(VISITOR* visitor, air::base::NODE_PTR node) {
    ...
    visitor->Context().Handle_variable(preg->Id(), SSA_SYM::NO_INDEX,
                                       preg->Type_id(), node, true);
  }
  ...
  RETV Handle_stpf(VISITOR* visitor, air::base::NODE_PTR node) {
    ...
    visitor->Context().Handle_variable(node->Preg_id(),
                                       node->Field_id().Index(),
                                       node->Child(0)->Rtype_id(), node, true);
  }
