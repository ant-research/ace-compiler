...
public:
  ...
  void Verify_stmt_def(SSA_SYM_ID sym, air::base::NODE_ID node) {
    ...
    _stack.Push_ver(ver);
    air::base::NODE_PTR node_ptr = _ssa_cont->Container()->Node(node);
    if (_ssa_cont->Has_chi(node_ptr)) {
...
public:
  ...
  void Rename_phi_opnd(air::base::NODE_PTR node, uint32_t opnd) {
    ...
    auto rename = [](PHI_NODE_PTR phi, const SSA_CONTAINER* cont,
      ...
      if (ver->Id() != stk.Top_ver_id(sym)) {
        SSA_VER_PTR top = cont->Ver(stk.Top_ver_id(sym));
        top->Print();
      }
