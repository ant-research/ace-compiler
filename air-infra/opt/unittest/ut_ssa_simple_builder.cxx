...
TEST_F(SSA_BUILDER_TEST, add_func) {
  ...
  air::base::NODE_PTR lda = cont->New_lda(
      func_scope->Formal(0), air::base::POINTER_KIND::FLAT32, spos);
  ...
  air::base::NODE_PTR add =
      cont->New_bin_arith(air::core::OPC_ADD, ldp1, ldp2, spos);
  ...
  ssa_builder.Perform();
  CHI_NODE_ID chi_id = ssa_cont.Node_chi(ist_arr0->Node()->Id());
  SSA_SYM_ID  sym_id = ssa_cont.Chi_node(chi_id)->Sym_id();
  SSA_SYM_PTR sym    = ssa_cont.Sym(sym_id);
  ASSERT_EQ(sym->Type_id(), Array_type()->Id());

  chi_id = ssa_cont.Node_chi(stf_fld3->Node()->Id());
  sym_id = ssa_cont.Chi_node(chi_id)->Sym_id();
  ...
  MU_NODE_ID  mu_id = ssa_cont.Node_mu(ild_arr0->Id());
  MU_NODE_PTR mu    = ssa_cont.Mu_node(mu_id);
  sym               = ssa_cont.Sym(mu->Sym_id());
  ASSERT_EQ(sym->Type_id(), Array_type()->Id());
