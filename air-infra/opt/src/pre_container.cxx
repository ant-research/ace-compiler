...
OCC_PTR PRE_CONTAINER::Append_real_occ(HCR_PTR cr, HSTMT_PTR sr) {
  PRE_CAND_PTR pre_cand = Find_or_new_pre_cand(cr);
  AIR_ASSERT(!pre_cand->Is_null());

  OCC_PTR exp_occ = New_real_occ(cr, sr);
  pre_cand->Append_occ(exp_occ);
  return exp_occ;
}

void PRE_CONTAINER::Append_phi_occ(HCR_PTR cr, BB_PTR bb, uint32_t num_opnd) {
  PRE_CAND_PTR pre_cand = Find_or_new_pre_cand(cr);
  AIR_ASSERT(!pre_cand->Is_null());

  PHI_OCC_DATA_PTR data_ptr = Static_cast<PHI_OCC_DATA_PTR>(
      _occ_tab->Malloc(PHI_OCC_DATA::Size(num_opnd)));
  new (data_ptr) PHI_OCC_DATA(cr, bb, num_opnd);
...
void PRE_CONTAINER::Append_phi_opnd_occ(OCC_PTR phi_occ, uint32_t phi_idx) {
  ...
  new (data_ptr) PHI_OPND_OCC_DATA(cr, pred_bb);
...
void PRE_CONTAINER::Insert_new_operand(OCC_PTR phi_opnd_occ) {
  AIR_ASSERT(phi_occ->Kind() == OCC_PHI_OCCUR);
  AIR_ASSERT(phi_opnd_occ->Kind() == OCC_PHI_OPND_OCCUR);

  PHI_OPND_OCC_DATA_PTR opnd_occ = phi_opnd_occ->Cast_to_phi_opnd_occ();
  HCR_ID insert_expr = opnd_occ->Cur_ver_cr();
