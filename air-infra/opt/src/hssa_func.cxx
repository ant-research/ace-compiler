...
NODE_PTR BB::Emit(HSSA_FUNC* hssa_func, NODE_PTR cur_blk, BBID_SET& visited) {
  ...
  HSSA_CONTAINER* hssa_cont = hssa_func->Hssa_cont();
  ...
  auto emit_stmt = [](HSTMT_PTR sr, CONTAINER* cont, STMT_LIST& stmt_list) {
    STMT_PTR stmt = sr->Emit(cont);
...
void LOOP_INFO::Emit(HSSA_FUNC* hssa_func, NODE_PTR blk, BBID_SET& visited) {
  ...
  AIR_ASSERT(visited.find(Cond_bb()->Id()) == visited.end());
  CONTAINER*      cont      = hssa_func->Output_cont();
  HSSA_CONTAINER* hssa_cont = hssa_func->Hssa_cont();
  STMT_LIST       stmt_list(blk);
  BB_PTR          init_bb = Init_bb();
  ...
  VAR_HCR_DATA_PTR iv = Ind_expr()->Cast_to_var_cr();
  AIR_ASSERT(iv->Var_kind() == VAR_KIND::ADDR_DATUM);
  ...
  Exit_bb()->Emit(hssa_func, blk, visited);
  // stmt_list.Append(exit_node->Stmt());

  visited.insert(Init_bb()->Id());
  visited.insert(Loop_body()->Id());
  visited.insert(Cond_bb()->Id());
}

NODE_PTR OP_HCR_DATA::Emit(CONTAINER* cont, HSSA_CONTAINER* hssa_cont) {
  TYPE_PTR rtype = cont->Glob_scope()->Type(Rtype());
  NODE_PTR node;
  // special handler for ARRAY
  if (Opcode() == air::core::ARRAY) {
    HCR_PTR kid0 = hssa_cont->Cr_ptr(Kid(0));
    if (kid0->Opcode() == air::core::LDCA) {
      ...
      HCR_PTR        addr_cr = kid0->Kid(0);
      uint32_t       addr_id = addr_cr->Cast_to_var_cr()->Var_id();
    ...
    cont->Set_array_idx(node, 0, hssa_cont->Cr_ptr(Kid(1))->Emit(cont));
  } else {
    node = cont->New_cust_node(Opcode(), rtype, Spos());
    for (uint32_t idx = 0; idx < Kid_cnt(); idx++) {
      NODE_PTR child = hssa_cont->Cr_ptr(Kid(idx))->Emit(cont);
...
NODE_PTR CST_HCR_DATA::Emit(CONTAINER* cont) {
...
NODE_PTR HCR::Emit(CONTAINER* cont) {
  NODE_PTR ret = Null_ptr;
  switch (Kind()) {
    case CK_OP:
      ret = Cast_to_op_cr()->Emit(cont, Hssa_cont());
      break;
    case CK_VAR: {
      ret = Cast_to_var_cr()->Emit_rhs(cont, Spos());
      break;
    }
    case CK_CONST:
      ret = Cast_to_cst_cr()->Emit(cont);
      break;
    case CK_IVAR:
    case CK_RCONST:
    case CK_LDA:
  ...
  ret->Set_attr_id(Attr());
  return ret;
}

STMT_PTR VAR_HCR_DATA::Emit_lhs(CONTAINER* cont, NODE_PTR rhs,
                                const SPOS& spos) {
  STMT_PTR ret = Null_ptr;
  if (Var_kind() == VAR_KIND::ADDR_DATUM) {
  ...
  } else if (Var_kind() == VAR_KIND::PREG) {
...
NODE_PTR VAR_HCR_DATA::Emit_rhs(CONTAINER* cont, const SPOS& spos) {
  NODE_PTR ret = Null_ptr;
  if (Var_kind() == VAR_KIND::ADDR_DATUM) {
    // TODO: var is global or local?
    ADDR_DATUM_PTR datum =
...
STMT_PTR HSTMT::Emit(CONTAINER* cont) {
  STMT_PTR        ret       = Null_ptr;
  ...
  switch (Kind()) {
    ...
    case SK_CALL: {
      CALL_HSTMT_DATA_PTR call_sr = Cast_to_call_sr();
      ENTRY_PTR entry   = cont->Glob_scope()->Entry_point(call_sr->Entry());
      PREG_PTR  retv    = air::base::Null_ptr;
      HCR_ID    retv_id = call_sr->Retv();
      if (retv_id != HCR_ID()) {
        VAR_HCR_DATA_PTR retv_var =
            hssa_cont->Cr_ptr(retv_id)->Cast_to_var_cr();
        AIR_ASSERT(retv_var->Var_kind() == VAR_KIND::PREG);
        retv = cont->Parent_func_scope()->Preg(PREG_ID(retv_var->Var_id()));
      }
      ret = cont->New_call(entry, retv, call_sr->Kid_cnt(), Spos());
      for (uint32_t idx = 0; idx < call_sr->Kid_cnt(); idx++) {
        HCR_ID   kid_id = call_sr->Kid(idx);
        HCR_PTR  kid    = _cont->Cr_ptr(kid_id);
        NODE_PTR child  = kid->Emit(cont);
    ...
    case SK_ASSIGN: {
      ASSIGN_HSTMT_DATA_PTR assign_sr = Cast_to_assign_sr();
      HCR_PTR               rhs       = hssa_cont->Cr_ptr(assign_sr->Rhs());
      NODE_PTR              rhs_node  = rhs->Emit(cont);

      HCR_PTR lhs = hssa_cont->Cr_ptr(assign_sr->Lhs());
      AIR_ASSERT(lhs->Kind() == CK_VAR);
      VAR_HCR_DATA_PTR lhs_var = lhs->Cast_to_var_cr();
