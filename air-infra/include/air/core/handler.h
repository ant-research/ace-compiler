...
public:
  ...
  template <typename RETV, typename VISITOR, typename NODE_TYPE>
  RETV Handle(VISITOR* visitor, NODE_TYPE node) {
    AIR_ASSERT(node->Domain() == ID);
    switch (node->Operator()) {
      // case definition from opcode_def.inc
#define DEF_OPCODE(NAME, name, kid_num, fld_num, prop) \
  OPCODE_HANDLER_GEN(NAME, name, kid_num, fld_num, prop)
...
#define DEF_OPCODE(NAME, name, kid_num, fld_num, prop) \
  OPCODE_IMPL_GEN_CR(NAME, name, kid_num, fld_num, prop)
#include "opcode_def.inc"
#undef DEF_OPCODE

#define DEF_OPCODE(NAME, name, kid_num, fld_num, prop) \
  OPCODE_IMPL_GEN_SR(NAME, name, kid_num, fld_num, prop)
#include "opcode_def.inc"
#undef DEF_OPCODE
