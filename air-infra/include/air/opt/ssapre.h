...
private:
  HSSA_CONTAINER& Hssa_cont() const { return _pre_cont.Hssa_cont(); }
  ...
  HCR_PTR         Cur_tmp() const { return _cur_tmp; }
  void            Set_cur_cand(PRE_CAND_PTR cand) { _pre_cand = cand; }
  void            Set_cur_tmp(HCR_PTR tmp) { _cur_tmp = tmp; }
  void            Inc_cur_ver(void) { _cur_ver++; }
  void            Init_cur_ver(void) { _cur_ver = 0; }
  BBID_CUS_SET&   Var_phi_set() { return _var_phi_set; }
  BBID_CUS_SET&   Df_phi_set() { return _df_phi_set; }
  ...
  void Get_domfrontier(BB_PTR bb, BBID_CUS_SET& bb_set);
  void Collect_phi_sets();
  void Create_phi_occs();
  void Create_phi_opnd_occs();
  void Gen_var_phi_list(HCR_PTR cr);
  void Build_all_occs();
  ...
  bool    Is_cr_modify_phi_res(HCR_PTR cr, BB_PTR phi_bb);
  bool    Need_fixing_phi(OCC_PTR def, OCC_PTR use);
  void    Create_new_version(OCC_PTR& occ, std::stack<OCC_PTR>& occ_stack);
  HCR_PTR Phi_opnd_with_cur_ver(OCC_PTR occ, uint32_t phi_idx);
  HCR_PTR Get_cur_ver(HCR_PTR expr, OCC_PTR phi_occ, uint32_t phi_idx);

  // Step 5: Finalize APIs
  bool    Need_insert(PHI_OPND_OCC_DATA_PTR phi_opnd);
  void    Compute_save_reload_inserts();
  bool    Replace_occurs(OCC_PTR occ, HCR_ID saved_cr);
  HCR_PTR Get_or_new_temp_var_exp(HCR_PTR cr);
  ...
  HCR_PTR       _cur_tmp;   // current tmp var cr, init for each pre candidate
  uint32_t      _cur_ver;   // current tmp version, init for each pre candidate
  ALL_OCC_LIST  _cur_all_occs;  // current all occs
  BBID_CUS_SET  _var_phi_set;   // phi inserted by var UD
  BBID_CUS_SET  _df_phi_set;    // phi  inserted by dominance frontiers
