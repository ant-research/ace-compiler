...
public:
  ...
  HSSA_CONTAINER& Hssa_cont() const { return _cfg.Hssa_cont(); }

  HCR_PTR Parent(size_t nth) const {
  ...
  template <typename RETV, typename VISITOR>
  RETV Handle_var(VISITOR* visitor, HCR_PTR node) {
    AIR_ASSERT(node->Kind() == CK_VAR);
    VAR_HCR_DATA_PTR var_cr = node->Cast_to_var_cr();
    if (var_cr->Def_by_stmt()) {
      HSTMT_PTR def_stmt = Hssa_cont().Stmt_ptr(var_cr->Def_stmt());
      ...
      if (_visited_phi.find(phi->Id()) == _visited_phi.end()) {
        ...
        for (uint32_t idx = 0; idx < phi->Size(); idx++) {
          ...
          // std::cout << "cr" << phi->Result_id().Value() << "--phi-->cr"
      ...
      HCHI_PTR chi = Hssa_cont().Chi_ptr(var_cr->Def_chi());
  ...
  template <typename RETV, typename VISITOR>
  RETV Handle_op(VISITOR* visitor, HCR_PTR node) {
    AIR_ASSERT(node->Kind() == CK_OP);
    OP_HCR_DATA_PTR op_cr = node->Cast_to_op_cr();
  ...
  template <typename RETV, typename VISITOR>
  RETV Handle_sr(VISITOR* visitor, HSTMT_PTR node) {
    if (Is_stop()) return RETV();
    switch (node->Kind()) {
      case SK_ASSIGN: {
        ASSIGN_HSTMT_DATA_PTR ass_stmt = node->Cast_to_assign_sr();
        return visitor->template Visit<RETV>(
            Hssa_cont().Cr_ptr(ass_stmt->Rhs()));
      }
      case SK_OP: {
        OP_HSTMT_DATA_PTR op_stmt = node->Cast_to_op_sr();
        ...
        for (size_t idx = 0; idx < kid_cnt; idx++) {
          visitor->template Visit<RETV>(Hssa_cont().Cr_ptr(op_stmt->Kid(idx)));
        }
      }
      case SK_CALL: {
        CALL_HSTMT_DATA_PTR call_stmt = node->Cast_to_call_sr();
        uint32_t            kid_cnt   = call_stmt->Kid_cnt();
        for (size_t idx = 0; idx < kid_cnt; idx++) {
          visitor->template Visit<RETV>(
              Hssa_cont().Cr_ptr(call_stmt->Kid(idx)));
        }
      }
      default:
  ...
  RETV Handle_unknown_domain(VISITOR* visitor, HSTMT_PTR node) {
    if (Is_stop()) return RETV();
    return visitor->Context().template Handle_sr<RETV, VISITOR>(visitor, node);
    // AIR_ASSERT_MSG(false, "Internal error: unknown domain");
    // return RETV();
  }

  template <typename RETV, typename VISITOR>
  RETV Handle_unknown_domain(VISITOR* visitor, HCR_PTR node) {
    if (Is_stop()) return RETV();
    return visitor->Context().template Handle_cr<RETV, VISITOR>(visitor, node);
    // AIR_ASSERT_MSG(false, "Internal error: unknown domain");
    // return RETV();
  }

  template <typename RETV, typename VISITOR>
  RETV Pre_handle_sr(VISITOR* visitor, HSTMT_PTR node) {
    if (Is_stop()) return RETV();
    Set_parent_stmt(node);
    // std::cout << "======================\n";
    // std::cout << "Enter sr[" << node->Id().Value() << "]" << std::endl;
    // node->Print();
    // std::cout << std::endl;
  ...
  template <typename RETV, typename VISITOR>
  RETV Pre_handle_cr(VISITOR* visitor, HCR_PTR node) {
    if (Is_stop()) return RETV();
    // std::cout << "======================\n";
    // std::cout << "Enter cr[" << node->Id().Value() << "]" << std::endl;
    // node->Print();
    // std::cout << std::endl;
  ...
  template <typename RETV, typename VISITOR>
  RETV Post_handle_sr(VISITOR* visitor, HSTMT_PTR node) {
    if (Is_stop()) return RETV();
    // std::cout << "Exit sr[" << node->Id().Value() << "]" << std::endl;
    return RETV();
  }
  ...
  RETV Post_handle_cr(VISITOR* visitor, HCR_PTR node) {
    if (Is_stop()) return RETV();
    // std::cout << "Exit cr[" << node->Id().Value() << "]" << std::endl;
    return RETV();
  }

  //! @brief define type for NODE_STACK
  using NODE_STACK = std::vector<HCR_PTR>;

  //! @brief guard to push/pop node automatically
  class GUARD {
  public:
    //! @brief constructor, push node onto stack
    GUARD(HSSA_UD_TRAV_CTX& ctx, const HCR_PTR& node)
  ...
  private:
    ...
    const HCR_PTR& _node;   // current node
  };                        // GUARD
