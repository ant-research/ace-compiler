...
public:
  ...
  HSSA_CONTAINER* Hssa_cont() const { return _hssa_cont; }
...
void HSSA_FUNC::Build(HSSA_VISITOR& visitor) {
  ...
  // build hssa & cfg
  _hssa_cont = _ma.Allocate<HSSA_CONTAINER>(cont, &ssa_cont);
  _cfg       = _ma.Allocate<CFG>(*_hssa_cont, cont->Entry_stmt()->Spos());
