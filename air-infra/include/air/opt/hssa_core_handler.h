...
public:
  ...
  RETV Handle_func_entry(VISITOR* visitor, air::base::NODE_PTR node) {
    ...
    HSSA_CONTAINER&   hssa_cont = ctx.Hssa_cont();
    CFG&              cfg       = ctx.Cfg();
    ...
    for (uint32_t i = 0; i < node->Num_child() - 1; ++i) {
      ...
      entry_stmt->Cast_to_op_sr()->Set_kid(i, child->Id());
    }
  ...
  RETV Handle_intconst(VISITOR* visitor, air::base::NODE_PTR node) {
    HSSA_CONTAINER& hssa_cont = visitor->Context().Hssa_cont();
    CST_HCR_DATA    cst_cr(node);
    HCR_DATA_PTR    cst_ptr(&cst_cr, HCR_ID());
    HCR_PTR ret = hssa_cont.Find_or_new_cr(HCR_PTR(HCR(&hssa_cont, cst_ptr)));
    return ret;
  }
  ...
  RETV Handle_st(VISITOR* visitor, air::base::NODE_PTR node) {
    ...
    HCR_PTR           rhs       = visitor->template Visit<RETV>(node->Child(0));
    SSA_VER_ID        lhs_ver_id = ssa_cont.Node_ver_id(node->Id());
    HCR_PTR           lhs        = hssa_cont.Find_or_new_var_cr(lhs_ver_id);
    stmt->Set_lhs(lhs->Id());
    stmt->Set_rhs(rhs->Id());
    lhs->Cast_to_var_cr()->Set_def_stmt(stmt->Id());
  ...
  RETV Handle_ld(VISITOR* visitor, air::base::NODE_PTR node) {
    ...
    HCR_PTR cr = ctx.Hssa_cont().Find_or_new_var_cr(ver_id);
    return cr;
  }

  // TODO: create new HCR type for LDA
  template <typename RETV, typename VISITOR>
  RETV Handle_lda(VISITOR* visitor, air::base::NODE_PTR node) {
    ...
    AIR_ASSERT(ver_id != SSA_VER_ID());
    HCR_PTR sym_cr = hssa_cont.Find_or_new_var_cr(ver_id);
    // treat symbol as
    uint32_t     kid_cnt = 1;
    OP_HCR_DATA* op_cr   = OP_HCR_DATA::Alloc(node->Num_child() + 1);
    new (op_cr) OP_HCR_DATA(node);
    HCR_DATA_PTR op_ptr(op_cr, HCR_ID());
    op_cr->Set_kid_cnt(node->Num_child() + 1);
    HCR_PTR ret = hssa_cont.Find_or_new_cr(HCR_PTR(HCR(&hssa_cont, op_ptr)));
    ret->Set_kid(0, sym_cr->Id());
    free(op_cr);
    return ret;
  }
  ...
  RETV Handle_call(VISITOR* visitor, air::base::NODE_PTR node) {
    ...
    CALL_HSTMT_DATA_PTR call_stmt = stmt->Cast_to_call_sr();
    if (ret_preg != air::base::Null_ptr) {
      SSA_VER_ID ver_id = ctx.Ssa_cont().Node_ver_id(node->Id());
      HCR_PTR    retv   = hssa_cont.Find_or_new_var_cr(ver_id);
      call_stmt->Set_retv(retv->Id());
    }

    for (uint32_t i = 0; i < node->Num_arg(); i++) {
      NODE_PTR arg      = node->Child(i);
      HCR_PTR  arg_expr = visitor->template Visit<RETV>(arg);
      call_stmt->Set_kid(i, arg_expr->Id());
    }
    ctx.Append_stmt(stmt);
    return RETV();
  }
  ...
  HPHI_PTR Create_phis(HSSA_BUILDER_CTX& ctx, BB_PTR bb, NODE_PTR node) {
    ...
    auto create_phi = [](PHI_NODE_PTR phi, BB_PTR bb,
  ...
  void Handle_phi_opnd(HSSA_BUILDER_CTX& ctx, HPHI_PTR hphi, PHI_NODE_ID phi_id,
    ...
    auto meet = [](PHI_NODE_PTR phi, HSSA_BUILDER_CTX& ctx, HPHI_PTR& hphi,
      ...
      HSSA_CONTAINER& hssa_cont = ctx.Hssa_cont();
      HCR_PTR         cr = hssa_cont.Find_or_new_var_cr(phi->Opnd_id(phi_idx));
      hphi->Set_opnd(phi_idx, cr->Id());
  ...
  void Handle_phi_res(HSSA_BUILDER_CTX& ctx, HPHI_PTR hphi,
                      PHI_NODE_ID phi_id) {
    auto meet = [](PHI_NODE_PTR phi, HSSA_BUILDER_CTX& ctx, HPHI_PTR& hphi) {
      HCR_PTR cr = ctx.Hssa_cont().Find_or_new_var_cr(phi->Result_id());
      cr->Set_flag(CF_DEF_BY_PHI);
      cr->Set_defphi(hphi);
      hphi->Set_result(cr->Id());
      if (hphi->Next_id() != HPHI_ID()) {
  ...
  RETV Handle_do_loop(VISITOR* visitor, air::base::NODE_PTR node) {
    ...
    HCR_PTR init_cr  = visitor->template Visit<RETV>(node->Child(0));
    HCR_PTR init_lhs = hssa_cont.Find_or_new_var_cr(
        ssa_cont.Node_ver(node->Child(0)->Id())->Id());
    HSTMT_PTR init_stmt = hssa_cont.New_assign_stmt(init_lhs, init_cr);
    init_stmt->Set_lhs(init_lhs->Id());
    init_stmt->Set_rhs(init_cr->Id());
    ...
    HCR_PTR             incr_cr  = visitor->template Visit<RETV>(step);
    HCR_PTR             incr_lhs = hssa_cont.Find_or_new_var_cr(
    ...
    HSTMT_PTR incr_stmt = hssa_cont.New_assign_stmt(incr_lhs, incr_cr);
    ...
    HCR_PTR   cond      = visitor->template Visit<RETV>(node->Child(1));
  ...
  RETV Handle_if(VISITOR* visitor, air::base::NODE_PTR node) {
    ...
    ctx.Set_cur_bb(cond_bb);
    HCR_PTR   cond      = visitor->template Visit<RETV>(node->Child(0));
