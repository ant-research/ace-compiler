...
public:
  ...
  // TODO: implement EK_LDA leaf expression (cf. Open64 CK_LDA) to store
  //       ADDR_DATUM_ID, offset, field_id directly without OP_DATA wrapper
  template <typename RETV, typename VISITOR>
  RETV Handle_lda(VISITOR* visitor, air::base::NODE_PTR node) {
    HCONTAINER&    hssa_cont  = visitor->Context().Hssa_cont();
    ADDR_DATUM_PTR addr_datum = node->Addr_datum();
    HEXPR_PTR      sym_expr   = hssa_cont.New_var_expr(addr_datum);
  ...
  RETV Handle_comment(VISITOR* visitor, air::base::NODE_PTR node) {
    // Skip comment nodes during HSSA building
    return RETV();
  }

  template <typename RETV, typename VISITOR>
  RETV Handle_pragma(VISITOR* visitor, air::base::NODE_PTR node) {
    // Skip pragma nodes during HSSA building
    return RETV();
  }
