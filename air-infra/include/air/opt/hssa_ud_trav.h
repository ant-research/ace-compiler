...
public:
  ...
  void Set_root_node(HCR_PTR node) { _root_node = node; }

  HCR_PTR Root_node(void) const { return _root_node; }

  template <typename RETV>
  RETV Start(HCR_PTR node) {
    Set_root_node(node);
...
private:
  ...
  template <typename RETV>
  RETV Pre_visit_node(HCR_PTR node) {
    return _ctx.template Pre_handle_cr<RETV, THIS_TYPE>(this, node);
  }

  template <typename RETV>
  RETV Pre_visit_node(HSTMT_PTR node) {
    return _ctx.template Pre_handle_sr<RETV, THIS_TYPE>(this, node);
  }

  template <typename RETV>
  RETV Post_visit_node(HCR_PTR node) {
    return _ctx.template Post_handle_cr<RETV, THIS_TYPE>(this, node);
  }
  template <typename RETV>
  RETV Post_visit_node(HSTMT_PTR node) {
    return _ctx.template Post_handle_sr<RETV, THIS_TYPE>(this, node);
  }
