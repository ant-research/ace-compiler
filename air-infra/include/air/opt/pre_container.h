...
public:
  ...
  OCC_PTR Append_real_occ(HCR_PTR cr, HSTMT_PTR sr);
  void    Append_phi_occ(HCR_PTR cr, BB_PTR bb, uint32_t num_opnd);
  void    Append_phi_opnd_occ(OCC_PTR phi_occ, uint32_t phi_idx);
  void    Insert_new_operand(OCC_PTR phi_occ, OCC_PTR phi_opnd_occ);
  OCC_PTR New_real_occ(HCR_PTR cr, HSTMT_PTR sr);
...
private:
  ...
  uint32_t Hash_exp(HCR_PTR cr);

  PRE_CAND_PTR New_pre_cand(HCR_PTR cr);

  PRE_CAND_PTR Find_or_new_pre_cand(HCR_PTR cr);
