...
public:
  ...
  void Trav(BB_PTR bb) {
    ...
    auto trav = [](HSTMT_PTR sr, THIS_TYPE& visitor) {
      visitor.Visit<RETV>(sr);
    };
  ...
  void Push(const HCR_PTR& cr, const HSTMT_PTR& stmt) { _ctx.Push(cr, stmt); }

  //! @brief manually pop a node from visiting stack
  //! @param node node to be poped from stack
  void Pop(const HCR_PTR& cr, const HSTMT_PTR& stmt) { _ctx.Pop(cr, stmt); }
