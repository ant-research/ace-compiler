...
public:
  ...
  HSTMT_PTR Stmt(HSSA_CONTAINER* cont) {
    AIR_ASSERT(Is_stmt());
    return cont->Stmt_ptr(_stmt);
  }
  HCHI_PTR Chi(HSSA_CONTAINER* cont) {
    AIR_ASSERT(Is_chi());
    return cont->Chi_ptr(_chi);
  }
  HPHI_PTR Phi(HSSA_CONTAINER* cont) {
    AIR_ASSERT(Is_phi());
...
class DU_INFO {
public:
  void Add_use(HCR_PTR expr, HSTMT_PTR stmt) {
    //CMPLR_ASSERT(false, "ALARM: TO IMPL");
  }
  void Add_use(HCR_PTR expr, HCHI_PTR chi) {
    //CMPLR_ASSERT(false, "ALARM: TO IMPL");
  }
  void Add_use(HCR_PTR expr, HPHI_PTR phi) {
    //CMPLR_ASSERT(false, "ALARM: TO IMPL");
  }
  void Remove_use(HCR_PTR expr, HSTMT_PTR stmt) {
    //CMPLR_ASSERT(false, "ALARM: TO IMPL");
  }
  void Remove_use(HCR_PTR expr, HPHI_PTR phi) {
    //CMPLR_ASSERT(false, "ALARM: TO IMPL");
  }
  USE_LIST* Uses(HCR_PTR expr) {
    //CMPLR_ASSERT(false, "ALARM: TO IMPL");
    return nullptr;
  }
