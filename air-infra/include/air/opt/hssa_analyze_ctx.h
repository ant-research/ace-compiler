...
public:
  ...
  void Push(const HCR_PTR& cr, const HSTMT_PTR& stmt) {
    _stack.push_back(std::make_pair(cr, stmt));
  }

  //! @brief manually pop a node from visiting stack
  //! @param node node to be poped from stack
  void Pop(const HCR_PTR& cr, const HSTMT_PTR& stmt) {
    AIR_ASSERT(!_stack.empty() && _stack.back().first == cr &&
  ...
  RETV Handle_sr(VISITOR* visitor, HSTMT_PTR node) {
    switch (node->Kind()) {
      case SK_ASSIGN: {
        ...
        visitor->template Visit<RETV>(Hssa_cont().Cr_ptr(ass_stmt->Rhs()));
        break;
      }
      case SK_OP: {
      ...
      case SK_IF: {
        IF_HSTMT_DATA_PTR if_stmt = node->Cast_to_if_sr();
        visitor->template Visit<RETV>(Hssa_cont().Cr_ptr(if_stmt->Cond()));
        break;
      }
      default:
  ...
  RETV Handle_unknown_domain(VISITOR* visitor, HSTMT_PTR node) {
    return Handle_sr<RETV, VISITOR>(visitor, node);
  ...
  RETV Handle_unknown_domain(VISITOR* visitor, HCR_PTR node) {
    return Handle_cr<RETV, VISITOR>(visitor, node);
    // AIR_ASSERT_MSG(false, "Internal error: unknown domain");
    // return RETV();
  }
  ...
  public:
    ...
    GUARD(HSSA_ANALYZE_CTX& ctx, const HCR_PTR& node, const HSTMT_PTR& stmt)
        : _stack(ctx.Node_stack()), _node_info(node, stmt) {
      _stack.push_back(std::make_pair(node, stmt));
    }

    //! @brief constructor, push node onto stack
    GUARD(HSSA_ANALYZE_CTX& ctx, const HCR_PTR& node)
