# config/default_options.py
"""
Default compiler options for test cases and compilation.
...
Usage:
    from ace.fhe.config.default_options import get_compile_options
...
# Key: model name (matches ModelSpec.name)
...
def get_compile_options(
    ...
    2. ModelSpec.compile_options (case_options)
    3. MODEL_SPECIFIC_OPTIONS[model_name]
    4. DEFAULT_COMPILE_OPTIONS (lowest priority)

    Args:
        model_name: Name of the model (ModelSpec.name)
        case_options: Options from ModelSpec.compile_options
