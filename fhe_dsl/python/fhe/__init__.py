...
Architecture (top-down):
    ...
    3. Library: FHE execution engines (antlib, phantom, acelib)
...
# ============================================================================
# 6. Spec - Unified Compilation Input Descriptor
# ============================================================================

import warnings as _warnings

# Legacy spec system (deprecated) — suppress the warning from the spec module
# itself during import.  We emit our own deprecation warning lazily via
# __getattr__ so that merely importing ace.fhe does not produce noise.
with _warnings.catch_warnings():
    _warnings.filterwarnings("ignore", category=DeprecationWarning,
                             message="ace.fhe.spec is deprecated")
    from .spec import (
        CompileSpec,
        ModelEntity,
        FuncEntity,
...
_DEPRECATED_SPEC_NAMES = {
    "CompileSpec", "ModelEntity", "FuncEntity",
    "CompileConfig", "InputSpec", "RuntimeConfig", "DatasetSource",
}


def __getattr__(name):
    """Emit a deprecation warning only when legacy spec names are accessed."""
    if name in _DEPRECATED_SPEC_NAMES:
        _warnings.warn(
            f"{name} is deprecated. Use ModelSpec/FuncSpec from "
            "ace.fhe.config.spec instead.",
            DeprecationWarning,
            stacklevel=2,
        )
        return globals()[name]
...
# New spec system (preferred)
from .config.spec import ModelSpec, FuncSpec
from .config.profiler import ReLUProfiler
...
from .profiler import profiler, FHEProfiler, ProfileResult
