...
class TorchTracedModel(CompilationUnit):
    ...
    def __init__(self, traced_model, input_names: List[str],
        ...
        self._relu_vr_data = relu_vr_data or {}  # {air_name: vr_value}
        ...
        for node in self.traced_model.graph.nodes:
            ...
            if node.op == 'call_function':
                ...
                if is_custom_op:
                    ...
                    # Inject relu_vr attribute for ReLU nodes if VR data is available
                    if self._relu_vr_data and op_name in ('relu', 'tensor.relu'):
                        onnx_name = metadata.get('onnx_name', '')
                        vr_value = None
                        if onnx_name and onnx_name in self._relu_vr_data:
                            vr_value = self._relu_vr_data[onnx_name]
                        elif onnx_name:
                            # Fallback: strip FX disambiguation suffix (_N) for shared modules
                            # e.g. "layer1_0_relu_1_Relu" -> "layer1_0_relu_Relu"
                            base_name = re.sub(r'_(\d+)_Relu$', r'_Relu', onnx_name)
                            if base_name != onnx_name and base_name in self._relu_vr_data:
                                vr_value = self._relu_vr_data[base_name]
                                logger.debug("VR fallback: %s -> %s", onnx_name, base_name)
                        if vr_value is not None:
                            attrs['relu_vr'] = vr_value
                            logger.info("ReLU VR: node=%s, vr=%.1f", onnx_name, vr_value)
                        else:
                            logger.warning("ReLU VR: node=%s NOT found in vr_data (available keys: %s)",
                                           onnx_name, list(self._relu_vr_data.keys())[:5])
