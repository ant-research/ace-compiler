...
"""
FuncSpec instances for sample functions.
"""
import torch

from ace.fhe.config.spec import FuncSpec
from ace.sample.funcs import (
...
def _make_func_spec(name, func, input_shapes, ir_ops=None):
    """Helper to build a function FuncSpec."""
    example_inputs = tuple(torch.randn(*shape, dtype=torch.float32) for shape in input_shapes)
...
BRANCH_EXECUTION_FUNC = _make_func_spec("branch_execution_func", branch_execution_func, [(1, 10), (1, 10)], ir_ops=["Add"])
