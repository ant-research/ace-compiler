...
air::base::NODE_PTR TENSOR_LEVEL_HANDLER::Process_op(
    ...
    if (opcode == air::base::OPCODE::INVALID) {
        spdlog::error("Invalid opcode for: {}", op_name);
    ...
    for (const auto& name : input_names) {
        ...
        if (input_node == air::base::Null_ptr) {
            spdlog::error("Failed to resolve input: {}", name);
            return air::base::NODE_PTR();
        }
    ...
    if (input_count == 3) {
        ...
        spdlog::error("Unsupported input count: {}", input_count);
        return air::base::NODE_PTR();
    }

    // Set attributes (only if opcode supports PROP_ATTR)
...
void TENSOR_LEVEL_HANDLER::Set_node_attrs(
    ...
    for (const auto& [key, value] : attrs) {
        ...
        try {
            ...
            node->Set_attr(key.c_str(), &double_val, 1);
            continue;
        } catch (const std::bad_any_cast&) {}
        ...
        try {
            auto vec_val = std::any_cast<std::vector<double>>(value);
            node->Set_attr(key.c_str(), vec_val.data(), vec_val.size());
            continue;
        } catch (const std::bad_any_cast&) {}
        ...
        spdlog::warn("Unknown attribute type: {}", key);
    }
}
