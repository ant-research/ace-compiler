...
#include "ace/frontend/core/attr_converter.h"
#include "ace/frontend/layers/tensor_level.h"
#include "ace/frontend/layers/tensor_level_handler.h"
...
void IR_BUILDER::Set_level(LEVEL_TYPE level) {
    if (_level.find(level) != _level.end()) {
        ...
        spdlog::info("Set level to: {}", Level_type_to_string(level));
    } else {
        spdlog::warn("Level not registered: {}", static_cast<int>(level));
    }
}
...
std::string IR_BUILDER::Add_operation(const std::string& op_name,
    ...
    if (!tensor_handler.Has_op(opcode_name)) {
        spdlog::warn("Operation not supported: {}", op_name);
        return "";
    }
    ...
    try {
        ...
        spdlog::error("TENSOR_LEVEL_HANDLER failed: {}", e.what());
    } catch (...) {
        spdlog::error("TENSOR_LEVEL_HANDLER failed with unknown error");
    }
...
at::Tensor IR_BUILDER::Build_op(const std::string& op_name,
    ...
    if (!level) {
        spdlog::warn("No current level handler");
        return at::Tensor();
    }
