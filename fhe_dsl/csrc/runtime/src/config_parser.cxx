...
RT_DATA_INFO create_rt_data_info_from_dict(const pybind11::dict& weight_dict) {
  RT_DATA_INFO info;

  static std::vector<std::string> file_names;
  static std::vector<std::string> file_uuids;

  if (weight_dict.contains("_file_name")) {
      file_names.push_back(weight_dict["_file_name"].cast<std::string>());
      info._file_name = file_names.back().c_str();
  } else {
      info._file_name = nullptr;
  }

  if (weight_dict.contains("_file_uuid")) {
      file_uuids.push_back(weight_dict["_file_uuid"].cast<std::string>());
      info._file_uuid = file_uuids.back().c_str();
  } else {
...
void Parse_and_set_config(const pybind11::dict& config_dict) {
  ...
  config._weight_info = create_rt_data_info_from_dict(weight_dict);
