...
def get_full_config_hash(compile_options: dict, build_options: dict) -> str:
    """Get combined hash of compile options + build options (Level 3)."""
    full_config = {
...
def get_cache_path(cache_key: str, input_tensors=None,
                   compile_options: dict = None, build_options: dict = None) -> Path:
    ...
    # Level 3: compile_options hash (for comparing different CKKS params)
...
def is_cache_valid(cache_key: str, input_tensors=None,
                   compile_options: dict = None, build_options: dict = None) -> bool:
...
def compile_with_cache(cache_key: str, compile_options: dict, build_options: dict,
                       input_tensors, compile_func, *args, **kwargs):
    """Execute compilation with automatic cache management.

    Three-level cache structure:
    - Level 1: entity-frontend-library-device
    - Level 2: input_shape_dtype
    - Level 3: full_config_hash (compile_options + build_options)
