...
class GraphTransformPass:
    ...
    def _handle_call_function(self, node: torch.fx.Node, graph: torch.fx.Graph) -> None:
        ...
        if custom_op is not None:
            ...
            node_path = node.name.replace("_", ".")
            onnx_node_name = f"/{node_path}/Add"
            node.meta['onnx_name'] = onnx_node_name
    ...
    def _generate_onnx_name(self, node: torch.fx.Node, op_name: str) -> str:
        """
        Generate ONNX-style node name with hierarchical path.
        ...
        if len(parts) > 1:
            ...
            for i in range(len(parts)):
                ...
                path_parts.append(cumulative)
            path_parts[-1] = parts[-1]
            onnx_node_name = "/" + "/".join(path_parts) + "/" + op_name.title().replace("_", "")
        else:
            onnx_node_name = f"/{node_path}/{op_name.title().replace('_', '')}"

        return onnx_node_name
    ...
    def _generate_module_onnx_name(self, node: torch.fx.Node, module_target: str, op_name: str) -> str:
        ...
        Args:
            node: FX node
            module_target: Module target string
            op_name: Operation type name
        ...
        suffix_match = re.search(r'_(\d+)$', node.name)
        ...
        if len(parts) > 1:
            ...
            for i in range(len(parts) - 1):
                cumulative = ".".join(parts[:i+1])
                path_parts.append(cumulative)
            path_parts.append(module_name + suffix)
            onnx_node_name = "/" + "/".join(path_parts) + "/" + op_name.title().replace("_", "")
        else:
            onnx_node_name = "/" + module_name + suffix + "/" + op_name.title().replace("_", "")
    ...
    def _handle_reshape_node(self, node: torch.fx.Node, traced_model, graph: torch.fx.Graph, method_name: str) -> None:
        ...
        """
        node_path = node.name.replace("_", ".")
        onnx_node_name = f"/{node_path}/Reshape"
        node.meta['onnx_name'] = onnx_node_name
