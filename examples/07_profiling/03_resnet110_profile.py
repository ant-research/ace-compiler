...
"""
Profiling Example 3: ResNet-110 FHE Inference with fhe.profiler

Demonstrates how to use the FHE profiling API to trace inference on a deeper
ResNet model (110 layers vs 20 layers). ResNet-110 has more residual blocks
and higher accuracy but longer inference time.
...
Usage:
    python 03_resnet110_profile.py
    python 03_resnet110_profile.py --library phantom --device cuda
    python 03_resnet110_profile.py --num-images 5 --trace-dir ./trace
"""
...
from ace import fhe
from ace.model.spec_resnet import RESNET110_CIFAR10
from ace.model.dataset import load_cifar10_images, CIFAR10_CLASSES


def compile_resnet110(library="antlib", device="cpu"):
    """Compile ResNet-110 with the given backend."""
    spec = RESNET110_CIFAR10
...
def main():
    parser = argparse.ArgumentParser(description="ResNet-110 FHE profiling with fhe.profiler")
    ...
    # Compile
    print(f"Compiling ResNet-110 ({args.library}/{args.device})...")
    print("Note: ResNet-110 is a deep model (110 layers) with higher accuracy")
    print("      but significantly longer FHE inference time than ResNet-20.")
    program = compile_resnet110(library=args.library, device=args.device)
