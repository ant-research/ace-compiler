...
void Lower_formal_type(GLOB_SCOPE* gscope, GLOB_SCOPE* new_sharding_gscope,
                       ARRAY_SHARDING& shmap, FUNCID_MAP& func_map) {
  ...
  for (; f_iter != f_end; ++f_iter) {
    ...
    for (; param_iter != param_end; ++param_iter) {
      ...
      std::vector<int64_t> xyz =
          shmap.Get_data_map(fscope->Id(), param->Name()->Id());
...
GLOB_SCOPE* Run_sharding_opt(GLOB_SCOPE* gscope, VECTOR_CTX& ctx,
  ...
  for (GLOB_SCOPE::FUNC_SCOPE_ITER it = gscope->Begin_func_scope();
    ...
    for (; (formal_itr != end_formal) && (new_formal_itr != end_new_formal);
      ...
      ctx_trans.Set_addr_datum_map(formal->Id(), new_formal->Id());
    }

    air::core::HANDLER_RETV retv =
        trav_trans.Visit<air::core::HANDLER_RETV>(body);
    ...
    std::vector<int64_t> xyz =
          shmap.Get_data_map(fscope->Id(), input_node->Addr_datum()->Name()->Id());
    nn::core::Set_input_scheme_attr(input_node, 1, xyz[1], xyz[2], 1);
